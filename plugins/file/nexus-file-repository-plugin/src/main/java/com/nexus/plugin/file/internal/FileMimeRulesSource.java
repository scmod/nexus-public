 ...
 */
public class FileMimeRulesSource
    implements MimeRulesSource
{
  ...
  public String getRuleForPath(String path) {
    ...
    if (path.toLowerCase().endsWith(".tgz")) {
      return FileRepository.TARBALL_MIME_TYPE;
    }
    if (!path.toLowerCase().contains(FileRepository.FILE_REGISTRY_SPECIAL)) {
      return FileRepository.JSON_MIME_TYPE;
    }
    return null;
  }
}
