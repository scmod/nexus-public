...
public class ComponentPlexusResourceIT extends NexusAppTestSupport {
	private AbstractComponentListPlexusResource getComponentPlexusResource()
			throws Exception {
		return (AbstractComponentListPlexusResource) this.lookup(
				PlexusResource.class, "ComponentPlexusResource");
	}

	private PlexusComponentListResourceResponse runGetForRole(String role)
			throws Exception {
		AbstractComponentListPlexusResource componentPlexusResource = this
				.getComponentPlexusResource();

		Request request = new Request();
		request.getAttributes().put(
				AbstractComponentListPlexusResource.ROLE_ID, role);

		return (PlexusComponentListResourceResponse) componentPlexusResource
				.get(null, request, null, null);
	}
	...
	public void testInvalidRole() throws Exception {
		try {
			...
			Assert.assertEquals("Expected a 404 error message.", 404, e
					.getStatus().getCode());
		}
	}

	@Test
	public void testValidRoleMultipleResults() throws Exception {
		PlexusComponentListResourceResponse result = runGetForRole(PlexusResource.class
				.getName());

		Assert.assertTrue(result.getData().size() > 1); // expected a bunch of
														// these thing, with new
														// ones being
		// added all the time.
		...
		for (PlexusComponentListResource resource : (List<PlexusComponentListResource>) result
				.getData()) {
	...
	@Test
	public void testValidRoleSingleResult() throws Exception {
		PlexusComponentListResourceResponse result = runGetForRole("TEST_ROLE");

		Assert.assertTrue(result.getData().size() == 1);

		PlexusComponentListResource resource = (PlexusComponentListResource) result
				.getData().get(0);
	...
	@Test
	public void testNullDescriptionAndHint() throws Exception {
	...
	@Test
	public void testEmptyDescriptionAndHint() throws Exception {
