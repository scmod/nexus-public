...
public class RolesPRTest extends AbstractSecurityRestTest {

	public void testPostWithSpace() throws Exception {
		PlexusResource resource = this.lookup(PlexusResource.class,
				"RoleListPlexusResource");
		...
		RoleResourceResponse roleResponse = (RoleResourceResponse) resource
				.post(null, request, response, roleRequest);
		...
		roleResponse = (RoleResourceResponse) resource.get(null, request,
				response, null);
		Assert.assertEquals("with spaces", roleResponse.getData().getId());
