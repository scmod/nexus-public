...
public class RestletClientInfoProviderTest {
	...
	/**
	 * Without any Shiro config around, Shiro is not viable. Anyway, from Shiro
	 * perspective (and hence, any Shiro using app's perspective), this is a
	 * fatal configuration error. See
	 * {@link UnavailableSecurityManagerException} javadoc.
	 * <p>
	 * TEST IGNORED for now, as actually it does not make any sense to test
	 * "broken configuration" state of Shiro itself. In those cases, Nexus would
	 * be broken too anyway. Note: when run in "solo" this test passes, but when
	 * run together with all other tests, there is configured Shiro present, and
	 * it fails.
	 */
	...
	public void withRestletRequest() {
		try {
			...
			final Request request = new Request(Method.GET,
					"http://repository.sonatype.org/foo/bar");
			request.getAttributes().put("org.restlet.http.headers", new Form()); // this
																					// happens
																					// in
																					// Restlet
																					// HTTP
			...
			request.getClientInfo().setAddress(
					"Planet Crypton's IP address in IPv8"); // this is a free
															// form string
			// actually
			...
			Assert.assertEquals("Planet Crypton's IP address in IPv8",
					ci.getRemoteIP());
		} finally {
			unmockSecurity();
