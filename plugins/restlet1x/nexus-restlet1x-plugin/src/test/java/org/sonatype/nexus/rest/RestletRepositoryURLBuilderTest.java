...
public class RestletRepositoryURLBuilderTest {
	...
	public void setUp() throws Exception {
		...
		Mockito.doReturn(M2Repository.class.getName()).when(repository)
				.getProviderRole();
		...
		Mockito.doReturn(M2GroupRepository.class.getName()).when(group)
				.getProviderRole();
		Mockito.doReturn(MOCK_REPO_ID).when(group).getPathPrefix();

		repositoryRegistry = Mockito.mock(RepositoryRegistry.class);
		Mockito.doReturn(repository).when(repositoryRegistry)
				.getRepository(MOCK_REPO_ID);
		Mockito.doReturn(group).when(repositoryRegistry)
				.getRepository(MOCK_GROUP_ID);
		...
		typeDescriptors.add(myHintRtd = new RepositoryTypeDescriptor(
				M2Repository.class, "my-hint", MOCK_PATH_PREFIX));
		typeDescriptors.add(invalidRtd = new RepositoryTypeDescriptor(
				M1Repository.class, "invalid", "invalid"));

		repositoryTypeRegistry = Mockito.mock(RepositoryTypeRegistry.class);
		Mockito.doReturn(myHintRtd)
				.when(repositoryTypeRegistry)
				.getRepositoryTypeDescriptor(M2Repository.class.getName(),
						"my-hint");
		Mockito.doReturn(invalidRtd)
				.when(repositoryTypeRegistry)
				.getRepositoryTypeDescriptor(M1Repository.class.getName(),
						"invalid");
		Mockito.doReturn(typeDescriptors).when(repositoryTypeRegistry)
	...
	public void testForceBaseUrlById() throws Exception {
		...
		Mockito.doReturn(GLOBAL_BASE_URL).when(globalRestApiSettings)
				.getBaseUrl();

		RestletRepositoryURLBuilder urlFinder = new RestletRepositoryURLBuilder(
				repositoryRegistry, repositoryTypeRegistry,
				globalRestApiSettings);

		Assert.assertEquals(GLOBAL_BASE_URL + "/content/" + MOCK_PATH_PREFIX
				+ "/" + MOCK_REPO_ID,
	...
	public void testBaseUrlGlobalRequest() throws Exception {
		// NEXUS-6045: Restlet1x request cannot come in from anywhere else than
		// /service/local anymore
		...
		try {
			...
			// NEXUS-6045: Restlet1x request will point to the webapp
			// root/content
			Assert.assertEquals("http://from/request/content/"
					+ MOCK_PATH_PREFIX + "/" + MOCK_REPO_ID,
					urlFinder.getRepositoryContentUrl(MOCK_REPO_ID));
		} finally {
