...
public class ForgotPasswordTest extends NexusAppTestSupport {
	...
	public void testRestPassword() throws Exception {
		...
		PlexusResource resetEmailPR = this.lookup(PlexusResource.class,
				"UserResetPlexusResource");

		Request request = new Request();
		Response response = new Response(request);
		request.getAttributes().put(UserResetPlexusResource.USER_ID_KEY,
				username);
		...
		// Sample body: Your password has been reset. Your new password is:
		// c1r6g4p8l7
		...
		if (index != -1) {
			password = body.substring(passwordStartIndex,
	...
	private void setupEmailConfig() throws IOException, XmlPullParserException {
		FileInputStream fis = new FileInputStream(this.getNexusConfiguration());
		try {
			...
			FileWriter writer = new FileWriter(this.getNexusConfiguration());
			try {
	...
	public void setUp() throws Exception {
		...
		// ServerSetup smtp = new ServerSetup( 1234, null,
		// ServerSetup.PROTOCOL_SMTP );
		ServerSetup smtp = new ServerSetup(this.emailServerPort, null,
				ServerSetup.PROTOCOL_SMTP);
