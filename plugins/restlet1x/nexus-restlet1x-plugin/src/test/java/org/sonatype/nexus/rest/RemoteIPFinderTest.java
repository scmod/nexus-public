...
public class RemoteIPFinderTest {
	...
	public void testResolveIP() {
		...
		Mockito.doReturn("")
				.when(form)
				.getFirstValue(
						org.sonatype.nexus.web.RemoteIPFinder.FORWARD_HEADER);
		...
		Mockito.doReturn("missing, 127.0.0.2, unknown, 127.0.0.1")
				.when(form)
				.getFirstValue(
						org.sonatype.nexus.web.RemoteIPFinder.FORWARD_HEADER);
		Assert.assertEquals("127.0.0.2", RemoteIPFinder.findIP(restlet));

		Mockito.doReturn("127.0.0.3, 127.0.0.2, 127.0.0.1")
				.when(form)
				.getFirstValue(
						org.sonatype.nexus.web.RemoteIPFinder.FORWARD_HEADER);
		Assert.assertEquals("127.0.0.3", RemoteIPFinder.findIP(restlet));

		Mockito.doReturn("localhost")
				.when(form)
				.getFirstValue(
						org.sonatype.nexus.web.RemoteIPFinder.FORWARD_HEADER);
		...
		Mockito.doReturn("client, proxy1, proxy2")
				.when(form)
				.getFirstValue(
						org.sonatype.nexus.web.RemoteIPFinder.FORWARD_HEADER);
		Assert.assertEquals(null, RemoteIPFinder.findIP(restlet));
		...
		// restlet1x clientInfo addresses are the last known upstream address +
		// reverse of XFF
		clientInfo.setAddresses(Arrays.asList("upstream", "proxy2", "proxy1",
				"client"));
		Mockito.doReturn("client, proxy1, proxy2")
				.when(form)
				.getFirstValue(
