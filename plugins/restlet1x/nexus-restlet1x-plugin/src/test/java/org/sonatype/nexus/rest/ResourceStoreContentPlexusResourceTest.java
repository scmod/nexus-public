 ...
 */
public class ResourceStoreContentPlexusResourceTest extends TestSupport {
	...
	public void setup() {
		...
		underTest = new AbstractResourceStoreContentPlexusResource(security,
				new BuildApplicationStatusSource(), views) {
			@Override
			...
			protected Logger getLogger() {
				return LoggerFactory
						.getLogger(ResourceStoreContentPlexusResourceTest.class);
			}

			@Override
			protected String getValidRemoteIPAddress(Request request) {
				// for test simplicity, to not have to mock layers of
				// HTTPCall/Request
				return "127.0.0.1";
			}

			protected Reference getContextRoot(Request request) {
				// unimportant, as we don't go anywhere else than resource
				// itself
				return reference;
			}
		...
		when(
				itemUid.getBooleanAttributeValue(IsRemotelyAccessibleAttribute.class))
				.thenReturn(true);

		when(collectionItem.getRepositoryItemUid()).thenReturn(itemUid);
		when(fileItem.getRepositoryItemUid()).thenReturn(itemUid);
		when(fileItem.getResourceStoreRequest()).thenReturn(
				new ResourceStoreRequest("/some/path"));
	...
	public void testNexus5155CacheHeadersForCollectionItems()
			throws ResourceException, NoSuchResourceStoreException,
			IOException, IllegalOperationException, ItemNotFoundException,
			AccessDeniedException {
		underTest.renderStorageCollectionItem(context, request, response,
				variant, resourceStore, collectionItem);

		verify(headers).add("Pragma", "no-cache");
		verify(headers).add("Cache-Control",
	...
	public void testNexus5155CacheHeadersForLinkedCollectionItems()
		...
		underTest.renderStorageLinkItem(context, request, response, variant,
				resourceStore, linkItem);
	...
	@Test
	public void testNexus5155OmitCacheHeadersForFileItems() throws Exception {
	...
	/**
	 * This test simulates the Request call in a way Restlet does when no
	 * if-modified-since condition is present. So we verify that created
	 * ResourceStoreRequest does not have condition set.
	 */
	@Test
	public void testNexus5704IfNoneMatchNoNPEWithNoETag() {
		...
		// we need more mock responses as getResourceStoreRequest tries to
		// gather
		...
		final ResourceStoreRequest rsr = underTest.getResourceStoreRequest(
				request, "/some/path");
	...
	/**
	 * This test simulates the Request call in a way Restlet would do when ETag
	 * is badly formatted (is not quoted): parsing of HTTP request with ETag
	 * without quotes will detect the presence of condition (Conditions
	 * if-modified-since will be non empty list), but the only one member of the
	 * list is actually {@code null} (as saw when debugged NEXUS-5704). So we
	 * verify that created ResourceStoreRequest does not have condition set AND
	 * we don't have NPE either.
	 */
	@Test
	public void testNexus5704IfNoneMatchNoNPEWithBadlyParsedETag() {
		final Conditions conditions = new Conditions();
		// ETag will be list with one element: null when ETag field is badly
		// formatted
		// (as seen from DEBUG)
	...
	/**
	 * This test simulates the Request in a way Restlet does: parsing of HTTP
	 * request with ETag with quotes will be OK. So we verify is it detected and
	 * added to created ResourceStoreRequest.
	 */
	@Test
	public void testNexus5704IfNoneMatchNoNPEWithParsedETag() {
		final Conditions conditions = new Conditions();
		conditions.setNoneMatch(Collections.singletonList(new Tag(
				"{SHA1{fake-sha1-string}}", false)));
		when(request.getConditions()).thenReturn(conditions);
	...
	/**
	 * With matching condition (client known ETag and Nexus ETag matches) should
	 * result in {@link ResourceException} carrying status 304 Not Modified.
	 */
	@Test
	public void testNexus5704renderStorageFileItemWithMatchingCondition()
		...
		try {
			...
			when(attributes.containsKey(StorageFileItem.DIGEST_SHA1_KEY))
					.thenReturn(true);
			when(attributes.get(StorageFileItem.DIGEST_SHA1_KEY)).thenReturn(
					"1234567890");
	...
	/**
	 * With non matching condition (client known ETag and Nexus ETag differs),
	 * the {@link StorageFileItemRepresentation} should be returned that wraps
	 * the passed in {@link StorageFileItem} instance.
	 */
	@Test
	public void testNexus5704renderStorageFileItemWithNonMatchingCondition()
			throws Exception {
		final ResourceStoreRequest rsr = fileItem.getResourceStoreRequest();
		rsr.setIfNoneMatch("{SHA1{1234567890}}"); // client "knows" hash
													// 1234567890
		when(attributes.containsKey(StorageFileItem.DIGEST_SHA1_KEY))
				.thenReturn(true);
		when(attributes.get(StorageFileItem.DIGEST_SHA1_KEY)).thenReturn(
				"0987654321"); // item hash is
		// "0987654321"

		final StorageFileItemRepresentation representation = (StorageFileItemRepresentation) underTest
				.renderStorageFileItem(request, fileItem);
