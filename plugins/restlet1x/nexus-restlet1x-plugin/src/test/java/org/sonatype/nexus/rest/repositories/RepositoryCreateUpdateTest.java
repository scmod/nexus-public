...
public class RepositoryCreateUpdateTest extends NexusAppTestSupport {
	...
	public void testCreate() throws Exception {
		...
		Assert.assertEquals(ChecksumPolicy.STRICT.name(),
				result.getChecksumPolicy());
		...
		Assert.assertEquals(RepositoryPolicy.RELEASE.name(),
				result.getRepoPolicy());
		Assert.assertEquals("proxy", result.getRepoType());

		Assert.assertEquals("http://foo.com/", result.getRemoteStorage()
				.getRemoteStorageUrl());

		AuthenticationSettings resultAuth = result.getRemoteStorage()
				.getAuthentication();
		...
		RemoteConnectionSettings resultCon = result.getRemoteStorage()
	...
	public void testUpdate() throws Exception {
		...
		originalResource.getRemoteStorage().setRemoteStorageUrl(
				"http://foo-new.com");

		AuthenticationSettings authSettings = originalResource
		...
		RemoteConnectionSettings connectionSettings = originalResource
		...
		RepositoryPlexusResource plexusResource = (RepositoryPlexusResource) this
				.lookup(PlexusResource.class,
						RepositoryPlexusResource.class.getName());
		...
		RepositoryResourceResponse repoResponse = (RepositoryResourceResponse) plexusResource
				.put(null, request, response, repoRequest);
		RepositoryProxyResource result = (RepositoryProxyResource) repoResponse
				.getData();
		...
		Assert.assertEquals("http://foo-new.com/", result.getRemoteStorage()
				.getRemoteStorageUrl());
		...
		Assert.assertEquals("userAgentString-new",
				resultCon.getUserAgentString());
	...
	private RepositoryProxyResource sendAndGetResponse() throws Exception {
		...
		repositoryResource.setWritePolicy(RepositoryWritePolicy.ALLOW_WRITE
				.name());
		...
		RepositoryListPlexusResource plexusResource = (RepositoryListPlexusResource) this
				.lookup(PlexusResource.class,
						RepositoryListPlexusResource.class.getName());

		Request request = buildRequest();
		Response response = new Response(request);

		RepositoryResourceResponse repoResponse = (RepositoryResourceResponse) plexusResource
				.post(null, request, response, repoRequest);
		RepositoryProxyResource result = (RepositoryProxyResource) repoResponse
				.getData();
	...
	public void testCreateAuthNoProxy() throws Exception {
		...
		RepositoryPlexusResource updateResource = (RepositoryPlexusResource) this
				.lookup(PlexusResource.class,
						RepositoryPlexusResource.class.getName());
		...
		repoResponse = (RepositoryResourceResponse) updateResource.put(null,
				request, response, repoResponse);
	...
	@Test
	public void testUpdateLocalStorage() throws Exception {

		RepositoryProxyResource originalResource = this.sendAndGetResponse();
		String newlocalStorage = originalResource.getDefaultLocalStorageUrl()
				.replaceAll(originalResource.getId(), "foo/bar");
		...
		Assert.assertEquals(newlocalStorage,
				result.getOverrideLocalStorageUrl());
