...
/**
 * REST resource to retrieve the tree of roles and privileges assigned to a
 * user.
 */
@Singleton
...
public class UserRoleTreePlexusResource extends AbstractSecurityPlexusResource {
	...
	public PathProtectionDescriptor getResourceProtection() {
		return new PathProtectionDescriptor("/role_tree/*",
	...
	public Object get(Context context, Request request, Response response,
		...
		try {
			...
			if (Boolean.parseBoolean(request.getResourceRef().getQueryAsForm()
					.getFirstValue("isRole"))) {
			...
			throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST,
					"Role: " + userId + " could not be found.");
		}
	}

	protected void handleUser(User user, AuthorizationManager authzManager,
			RoleTreeResourceResponse response) {
			...
			try {
				...
				getLogger().debug(
						"Invalid roleId: " + roleIdentifier.getRoleId()
								+ " from source: " + roleIdentifier.getSource()
								+ " not found.");
			}
		}
	}

	protected void handleRole(Role role, AuthorizationManager authzManager,
			RoleTreeResourceResponse response, RoleTreeResource resource) {
		...
		for (String privilegeId : role.getPrivileges()) {
			try {
				Privilege childPrivilege = authzManager
