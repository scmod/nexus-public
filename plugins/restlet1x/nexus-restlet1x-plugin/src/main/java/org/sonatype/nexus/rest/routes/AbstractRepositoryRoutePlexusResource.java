 ...
 * @author cstamas
 */
public abstract class AbstractRepositoryRoutePlexusResource extends
	...
	@Inject
	public void setRepositoryMapper(
			final RequestRepositoryMapper repositoryMapper) {
	...
	/**
	 * Creating a list of member reposes. Since this method is used in two
	 * Resource subclasses too, and those are probably mapped to different
	 * bases, a listBase param is needed to generate a correct URI, from the
	 * actual subclass effective mapping.
	 */
	protected List<RepositoryRouteMemberRepository> getRepositoryRouteMemberRepositoryList(
			Reference listBase, List<String> reposList, Request request,
			String mapId) throws NoSuchRepositoryAccessException {
		List<RepositoryRouteMemberRepository> members = new ArrayList<RepositoryRouteMemberRepository>(
				reposList.size());
		...
		for (String repoId : reposList) {
			...
			if ("*".equals(repoId)) {
				...
				try {
					member.setName(getRepositoryRegistry()
							.getRepository(repoId).getName());
				} catch (NoSuchRepositoryAccessException e) {
					throw e;
				} catch (NoSuchRepositoryException e) {
					getLogger().warn(
							"Cannot find repository '" + repoId
									+ "' declared within route: + '" + mapId
									+ "'!", e);
					continue;
				}

				member.setResourceURI(createChildReference(request, this,
						repoId).toString());
			}
