...
public abstract class AbstractUIPermissionCalculatingPlexusResource extends
	...
	protected AuthenticationClientPermissions getClientPermissionsForCurrentUser(
		...
		if (getSecuritySystem().isAnonymousAccessEnabled()) {
			perms.setLoggedIn(!getSecuritySystem().getAnonymousUsername()
		...
		if (StringUtils.isNotEmpty(username)) {
			...
			try {
				...
				if (getLogger().isDebugEnabled()) {
					...
					getLogger().info("Failed to lookup user: {}: {}/{}",
	...
	private void checkSubjectsPermissions(Subject subject,
		...
		for (Entry<String, Integer> priv : privilegeMap.entrySet()) {
			...
			permissionList
					.add(new WildcardPermission(priv.getKey() + ":create"));
			permissionList
					.add(new WildcardPermission(priv.getKey() + ":update"));
			permissionList
					.add(new WildcardPermission(priv.getKey() + ":delete"));
		...
		if (subject != null) {
			...
			// now loop through the original set and figure out the correct
			// value
			for (Entry<String, Integer> priv : privilegeMap.entrySet()) {
