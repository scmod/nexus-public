...
@Named
@Singleton
public class RestletRepositoryURLBuilder extends ComponentSupport implements
	...
	@Override
	public String getRepositoryContentUrl(final String repositoryId,
			final boolean forceBaseURL) throws NoSuchRepositoryException {
		return getRepositoryContentUrl(
				repositoryRegistry.getRepository(repositoryId), forceBaseURL);
	}
	...
	public String getRepositoryContentUrl(final Repository repository,
		...
		final boolean shouldForceBaseUrl = forceBaseURL
				|| (globalRestApiSettings.isEnabled()
						&& globalRestApiSettings.isForceBaseUrl() && StringUtils
							.isNotBlank(globalRestApiSettings.getBaseUrl()));

		String baseURL;

		// if force, always use force
		if (shouldForceBaseUrl) {
			baseURL = globalRestApiSettings.isEnabled() ? globalRestApiSettings
					.getBaseUrl() : null;
		}
		// next check if this thread has a restlet request
			...
			baseURL = Request.getCurrent().getRootRef().getParentRef()
		...
		if (StringUtils.isBlank(baseURL)) {
			log.info(
					"Not able to build content URL of the repository {}, baseUrl not set!",
		...
		final RepositoryTypeDescriptor rtd = repositoryTypeRegistry
				.getRepositoryTypeDescriptor(repository.getProviderRole(),
						repository.getProviderHint());

		url.append("content/").append(rtd.getPrefix()).append("/")
				.append(repository.getPathPrefix());
