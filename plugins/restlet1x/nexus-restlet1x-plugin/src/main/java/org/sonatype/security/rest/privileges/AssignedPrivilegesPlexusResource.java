...
/**
 * REST resource to retrieve the list of Privileges assigned to a user. Will
 * also include a tree that details how the privilege is assigned to this user
 * (through roles).
 */
@Singleton
...
public class AssignedPrivilegesPlexusResource extends
		AbstractSecurityPlexusResource {
	public static final String USER_ID_KEY = "userId";

	public static final String RESOURCE_URI = "/assigned_privileges/{"
			+ USER_ID_KEY + "}";
	...
	public PathProtectionDescriptor getResourceProtection() {
		return new PathProtectionDescriptor("/assigned_privileges/*",
	...
	public Object get(Context context, Request request, Response response,
		...
		try {
			...
			AuthorizationManager authzManager = getSecuritySystem()
					.getAuthorizationManager("default");

			for (RoleIdentifier roleIdentifier : user.getRoles()) {
				try {
					handleRole(
							authzManager.getRole(roleIdentifier.getRoleId()),
							null, authzManager, responseResource);
				} catch (NoSuchRoleException e) {
					getLogger().debug(
							"Invalid roleId: " + roleIdentifier.getRoleId()
									+ " from source: "
									+ roleIdentifier.getSource()
									+ " not found.");
				}
			}
			...
			throw new ResourceException(Status.SERVER_ERROR_INTERNAL,
					"Unable to load default authorization manager");
		}
	}

	protected void handleRole(Role role, List<Role> parentList,
		...
		for (String roleId : role.getRoles()) {
			try {
				handleRole(authzManager.getRole(roleId), newParentList,
						authzManager, response);
			} catch (NoSuchRoleException e) {
				getLogger()
						.debug("handleRole() failed, roleId: " + roleId
								+ " not found");
			}
		}
		...
		for (String privilegeId : role.getPrivileges()) {
			try {
				handlePrivilege(authzManager.getPrivilege(privilegeId),
						newParentList, response);
			} catch (NoSuchPrivilegeException e) {
				getLogger().debug(
						"handleRole() failed, privilegeId: " + privilegeId
								+ " not found");
			}
		}
	}
