...
public abstract class AbstractResourceStoreContentPlexusResource extends
	...
	@Inject
	public void setApplicationStatusSource(
			final ApplicationStatusSource applicationStatusSource) {
		this.applicationStatusSource = applicationStatusSource;
	}

	@Inject
	public void setViewProviders(
			final Map<String, ArtifactViewProvider> viewProviders) {
		this.viewProviders = viewProviders;
	}
	...
	protected String getResourceStorePath(Request request) {
		try {
			// #getRemainingPart(true, true) would use Restlet decoding, that
			// relies
			...
			final String remainingDecodedPart = URLDecoder.decode(
					remainingPart.replace("+", "%2B"), "UTF-8").replace("%2B",
					"+");
			return parsePathFromUri(remainingDecodedPart);
		} catch (UnsupportedEncodingException e) {
			throw new IllegalStateException(
					"Nexus cannot operate on platform not supporting UTF-8 character encoding!",
					e);
		}
	}
	...
	protected boolean isDescribe(Request request) {
		...
		return request.getResourceRef().getQueryAsForm()
				.getFirst(IS_DESCRIBE_PARAMETER) != null;
	}
	...
	public Object get(Context context, Request request, Response response,
		...
		try {
			...
			try {
				...
				return renderItem(context, request, response, variant, store,
						item);
			} catch (ItemNotFoundException e) {
				if (isDescribe(request)) {
					return renderDescribeItem(context, request, response,
							variant, store, req, null, e);
				} else {
					throw e;
				}
			}
	...
	private void addNoCacheHeaders(final Response response) {
		...
		final Series<Parameter> headers = ((HttpResponse) response)
				.getHttpCall().getResponseHeaders();
		headers.add("Pragma", "no-cache"); // HTTP/1.0
		headers.add("Cache-Control",
				"no-cache, no-store, max-age=0, must-revalidate"); // HTTP/1.1
		headers.add("Cache-Control", "post-check=0, pre-check=0"); // MS IE
		headers.add("Expires", "0"); // No caching on Proxies in between client
										// and Nexus
	}
	...
	public Object upload(Context context, Request request, Response response,
		...
		// NEXUS-4151: Do not accept upload/deploy requests with media type
		// (Content-Type) of
		// "application/x-www-form-urlencoded", since ad 1, it's wrong, ad 2, we
		// do know
		// Jetty's Request object "eats" up it's body to parse request
		// parameters, invoked
		...
		try {
			...
			for (FileItem fileItem : files) {
				getResourceStore(request).storeItem(req,
						fileItem.getInputStream(), null);
			}
		} catch (Exception t) {
	...
	public void delete(Context context, Request request, Response response)
		...
		try {
			...
			getLogger()
					.info("Storage item(s) on path \""
							+ req.getRequestPath()
							+ "\" (and below) were deleted from repository ["
							+ request
									.getAttributes()
	...
	/**
	 * A strategy to get ResourceStore implementor. To be implemented by
	 * subclass.
	 */
	 ...
	 * various things in Request context, like authenticated username, etc.
	 */
	...
	protected ResourceStoreRequest getResourceStoreRequest(Request request,
		...
		ResourceStoreRequest result = new ResourceStoreRequest(
				resourceStorePath);

		getLogger().trace("Created ResourceStore request for {}",
		...
		if (isDescribe(request)) {
			result.getRequestContext().put(REQUEST_RECEIVED_KEY,
					System.currentTimeMillis());
		}

		// honor if-modified-since
		if (request.getConditions().getModifiedSince() != null) {
			result.setIfModifiedSince(request.getConditions()
					.getModifiedSince().getTime());
		}
		...
		if (request.getConditions().getNoneMatch() != null
			...
			// NEXUS-5704: 500 Internal Server Error when "If-None-Match" in
			// header
			// Restlet 1.1 is very strict about properly formatted ETags (must
			// be quoted)
			// If unquoted, their presence is detected (IF above evals to true),
			// but will
	...
	protected Object renderItem(Context context, Request req, Response res,
			Variant variant, ResourceStore store, StorageItem item)
			throws IOException, AccessDeniedException,
			NoSuchResourceStoreException, IllegalOperationException,
			ItemNotFoundException, StorageException, ResourceException {
		if (isDescribe(req)) {
			return renderDescribeItem(context, req, res, variant, store,
					item.getResourceStoreRequest(), item, null);
		}

		if (!item.isVirtual()) {
			if (!item.getRepositoryItemUid().getBooleanAttributeValue(
					IsRemotelyAccessibleAttribute.class)) {
				getLogger()
						.debug(String
								.format("Request for remotely non-accessible UID %s is made and refused",
		...
		if (item instanceof StorageFileItem) {
			...
			return renderStorageLinkItem(context, req, res, variant, store,
					(StorageLinkItem) item);
		} else if (item instanceof StorageCollectionItem) {
			return renderStorageCollectionItem(context, req, res, variant,
					store, (StorageCollectionItem) item);
		}
	...
	@VisibleForTesting
	Object renderStorageCollectionItem(final Context context,
			final Request req, final Response res, final Variant variant,
			final ResourceStore store, final StorageCollectionItem coll)
			throws IOException, NoSuchResourceStoreException,
		...
		if (!resPath.endsWith("/")) {
			res.redirectPermanent(createRedirectReference(req).getTargetRef()
					.toString() + "/");
		...
		if (Method.HEAD.equals(req.getMethod())) {
			return renderHeadResponseItem(context, req, res, variant, store,
					coll.getResourceStoreRequest(), coll);
		}
		...
		for (StorageItem child : children) {
			if (child.isVirtual()
					|| !child.getRepositoryItemUid().getBooleanAttributeValue(
				...
				if (!uniqueNames.contains(child.getName())) {
					...
					resource.setLeaf(!StorageCollectionItem.class
							.isAssignableFrom(child.getClass()));

					String uri = getResourceUri(req, resource, child);
					resource.setResourceURI(uri);

					resource.setRelativePath(child.getPath()
							+ (resource.isLeaf() ? "" : "/"));

					resource.setLastModified(new Date(child.getModified()));

					resource.setSizeOnDisk(StorageFileItem.class
							.isAssignableFrom(child.getClass()) ? ((StorageFileItem) child)
							.getLength() : -1);
	...
	@VisibleForTesting
	Object renderStorageLinkItem(final Context context, final Request req,
			final Response res, final Variant variant,
			final ResourceStore store, final StorageLinkItem item)
			throws ResourceException {
		// we have a link, dereference it
		// TODO: we should be able to do HTTP redirects too! (parametrize the
		// dereferencing?)
		try {
			return renderItem(context, req, res, variant, store,
					getRepositoryRouter().dereferenceLink(item));
		} catch (Exception e) {
	...
	@VisibleForTesting
	Representation renderStorageFileItem(final Request req,
			final StorageFileItem file) throws ResourceException {
		final StorageFileItemRepresentation fileRepresentation = new StorageFileItemRepresentation(
				file);
		if (file.getResourceStoreRequest().getIfModifiedSince() != 0
				&& file.getModified() <= file.getResourceStoreRequest()
						.getIfModifiedSince()) {
			...
			throw new ResourceException(Status.REDIRECTION_NOT_MODIFIED,
				...
				&& file.getResourceStoreRequest().getIfNoneMatch()
						.equals(fileRepresentation.getTag().getName())) {
	...
	private String getResourceUri(Request req, ContentListResource resource,
			StorageItem child) {
		// NEXUS-4244: simply force both baseURLs, coming from nexus.xml and
		// extracted from current request
		// to end with slash ("/").
		Reference root = getContextRoot(req);
		if (StringUtils.isBlank(root.getPath())
				|| !root.getPath().endsWith("/")) {
			root.setPath(StringUtils.defaultString(root.getPath(), "") + "/");
		}
		Reference requestRoot = req.getRootRef().getParentRef().getParentRef();
		if (StringUtils.isBlank(requestRoot.getPath())
				|| !requestRoot.getPath().endsWith("/")) {
			requestRoot.setPath(StringUtils.defaultString(
					requestRoot.getPath(), "") + "/");
		}
	...
	protected Representation serialize(Context context, Request req,
			Variant variant, Object payload) throws IOException {
		...
		if (MediaType.TEXT_HTML.equals(variant.getMediaType())) {
			...
			dataModel
					.put("listItems",
							sortContentListResource(((ContentListResourceResponse) payload)
									.getData()));
			dataModel.put("request", req);
			dataModel.put("nexusVersion", applicationStatusSource
			...
			final VelocityRepresentation representation = new VelocityRepresentation(
					context, "/templates/repositoryContentHtml.vm", getClass()
							.getClassLoader(), dataModel,
	...
	protected Object renderHeadResponseItem(Context context, Request req,
			Response res, Variant variant, ResourceStore store,
			ResourceStoreRequest request, StorageCollectionItem coll)
			throws IOException, AccessDeniedException,
			NoSuchResourceStoreException, IllegalOperationException,
			ItemNotFoundException, StorageException, ResourceException {
		// we are just returning anything, the connector will strip off content
		// anyway.
		return new StorageItemRepresentation(variant.getMediaType(), coll);
	}

	protected Object renderDescribeItem(Context context, Request req,
			Response res, Variant variant, ResourceStore store,
			ResourceStoreRequest request, StorageItem item, Throwable t)
			throws IOException, AccessDeniedException,
			NoSuchResourceStoreException, IllegalOperationException,
			ItemNotFoundException, StorageException, ResourceException {
		Parameter describeParameter = req.getResourceRef().getQueryAsForm()
				.getFirst(IS_DESCRIBE_PARAMETER);
		...
		if (StringUtils.isNotEmpty(describeParameter.getValue())) {
			...
			if (!viewProviders.containsKey(key)) {
				throw new IllegalRequestException(request, "No view for key: "
						+ key);
			}

			Object result = viewProviders.get(key).retrieveView(store, request,
					item, req);

			// make sure we have valid content
			if (result == null) {
				throw new ItemNotFoundException(reasonFor(request,
						"View provider keyed \"%s\" did not provide content.",
						key));
			} else {
				return result;
			}
		...
		if (request.getRequestContext().containsKey(REQUEST_RECEIVED_KEY)) {
			long received = (Long) request.getRequestContext().get(
					REQUEST_RECEIVED_KEY);

			resource.setProcessingTimeMillis(System.currentTimeMillis()
					- received);
		} else {
		...
		resource.setResponse(describeResponse(context, req, res, variant,
				request, item, t));
	...
	protected ContentListDescribeRequestResource describeRequest(
			Context context, Request req, Response res, Variant variant,
		...
		for (Map.Entry<String, Object> entry : request.getRequestContext()
				.flatten().entrySet()) {
	...
	protected NotFoundReasoning buildNotFoundReasoning(
			final Repository repository, final Throwable t) {
		...
		if (t instanceof GroupItemNotFoundException) {
			...
			for (Map.Entry<Repository, Throwable> r : ginf.getMemberReasons()
					.entrySet()) {
				reasoning.addNotFoundReasoning(buildNotFoundReasoning(
						r.getKey(), r.getValue()));
			}
		}
	...
	protected ContentListDescribeResponseResource describeResponse(
			Context context, Request req, Response res, Variant variant,
			ResourceStoreRequest request, StorageItem item, Throwable e) {
		ContentListDescribeResponseResource result = new ContentListDescribeResponseResource();

		result.getProcessedRepositoriesList().addAll(
				request.getProcessedRepositories());

		// applied mappings
		for (Map.Entry<String, List<String>> mappingEntry : request
				.getAppliedMappings().entrySet()) {
			result.addAppliedMapping(mappingEntry.getKey()
					+ " repository applied " + mappingEntry.getValue());
		}
		...
		if (!item.isVirtual()) {
			...
			result.setOriginatingRepositoryId(item.getRepositoryItemUid()
					.getRepository().getId());

			result.setOriginatingRepositoryName(item.getRepositoryItemUid()
					.getRepository().getName());

			result.setOriginatingRepositoryMainFacet(item
					.getRepositoryItemUid().getRepository().getRepositoryKind()
	...
	private List<ContentListResource> sortContentListResource(
			Collection<ContentListResource> list) {
		List<ContentListResource> result = new ArrayList<ContentListResource>(
				list);
			...
			public int compare(ContentListResource o1, ContentListResource o2) {
				if (!o1.isLeaf()) {
					if (!o2.isLeaf()) {
						...
						// first item is a dir, second is a file, dirs always
						// win
						return 1;
					}
				} else if (!o2.isLeaf()) {
	...
	/**
	 * ResourceStore iface is pretty "chatty" with Exceptions. This is a
	 * centralized place to handle them and convert them to proper HTTP status
	 * codes and response.
	 */
	protected void handleException(final Request req, final Response res,
			final Exception t) throws ResourceException {
		// just set this flag to true in any if-else branch you want to see
		// complete error loglines with stack traces.
		// Note: when Nexus is in DEBUG logging mode, then we log _all_
		// exceptions
		...
		try {
			if (t instanceof ResourceException) {
				...
				// in case client drops connection, this makes not much sense,
				// as he will not
				// receive this response, but we have to end it somehow.
				// but, in case when remote proxy peer drops connection on us
				// regularly
				// this makes sense
				throw new ResourceException(getStatus(
						Status.CLIENT_ERROR_NOT_FOUND, t), t);
			} else if (t instanceof IllegalArgumentException) {
				throw new ResourceException(getStatus(
						Status.CLIENT_ERROR_BAD_REQUEST, t), t);
				...
				throw new ResourceException(getStatus(
						Status.CLIENT_ERROR_FORBIDDEN, t), t);
				...
				throw new ResourceException(new Status(ex.getResponseCode(),
						ex.getReasonPhrase(), ex.getMessage(), null), t);
			} else {
			...
			if (t instanceof ResourceException) {
				...
				// See NEXUS-4380
				// do not spam the log with non-error responses, we need only
				// errors to have logged in case when
				// exception to be handled is ResourceException
				if (re.getStatus() == null || re.getStatus().isError()) {
					handleErrorConstructLogMessage(req, res, t,
	...
	protected void handleErrorConstructLogMessage(final Request req,
			final Response res, final Exception t,
			final boolean shouldLogInfoStackTrace) {
		String message = "Got exception during processing request \""
				+ req.getMethod() + " " + req.getResourceRef().toString()
				+ "\": ";
		...
		if (t instanceof NoSuchRepositoryException
			...
			// if DEBUG level, we log _all_ errors with stack traces, except the
			// ItemNotFoundException

			if (t instanceof ItemNotFoundException) {
				// we are "muting" item not found exception stack traces, it
				// pollutes the DEBUG logs
			...
			// if not in DEBUG mode, we obey the flag to decide whether we need
			// to log or not the stack trace
			if ((t instanceof ItemNotFoundException || t instanceof IllegalRequestException)
					&& !shouldLogInfoStackTrace) {
				// mute it
			} else {
				if (shouldLogInfoStackTrace) {
					// in INFO mode, we obey the shouldLogInfoStackTrace flag
					// for serious errors (like internal is)
					getLogger().error(message, t);
				} else {
	...
	/**
	 * TODO: this code below should be removed. It is better than the one before
	 * (it actually reuses Shiro filter, and will not try to reassemble the
	 * Challenge anymore, but stil...
	 */
	public static void challengeIfNeeded(final Request req, final Response res,
			final AccessDeniedException t) {
		...
		// req ).getHttpCall() ).getRequest();
		//
		// if ( servletRequest.getAttribute(
		// NexusHttpAuthenticationFilter.ANONYMOUS_LOGIN ) != null )
		// {
		// // setting this flag to notify NexusHttpAuthenticationFilter that a
		// challenge is needed
		// // without actually _knowing_ what kind of challenge we use
		// servletRequest.setAttribute(
		// NexusJSecurityFilter.REQUEST_IS_AUTHZ_REJECTED, Boolean.TRUE );
		//
