...
/**
 * REST resource that searches for users based on a users source and other
 * search criteria.
 *
 * @author bdemers
 */
...
public class UserSearchPlexusResource extends AbstractUserSearchPlexusResource {
	public static final String USER_ID_KEY = "userId";

	public static final String USER_SOURCE_KEY = "userSource";

	public static final String RESOURCE_URI = "/user_search/{"
			+ USER_SOURCE_KEY + "}";
	...
	public PathProtectionDescriptor getResourceProtection() {
		return new PathProtectionDescriptor("/user_search/**",
	...
	public Object put(Context context, Request request, Response response,
		...
		UserSearchCriteria criteria = this
				.toPlexusSearchCriteria(criteriaResource);
		criteria.setSource(this.getUserSource(request));

		return this.search(criteria);
	}

	private UserSearchCriteria toPlexusSearchCriteria(
			PlexusUserSearchCriteriaResource criteriaResource) {
		UserSearchCriteria criteria = new UserSearchCriteria();
		criteria.setUserId(criteriaResource.getUserId());

		// NOTE: in the future we could expand the REST resource to send back a
		// list of roles, (or a single role)
		...
		if (criteriaResource.isEffectiveUsers()) {
			...
			try {
				...
				this.getLogger()
						.error("Cannot find default UserManager,  effective user search may not work properly.",
								e);
