...
public abstract class AbstractArtifactPlexusResource extends
	 ...
	 */
	protected ArtifactStoreRequest getResourceStoreRequest(Request request,
		...
		if (StringUtils.isBlank(e)) {
			e = mavenRepository.getArtifactPackagingMapper()
		...
		ArtifactStoreRequest result = new ArtifactStoreRequest(mavenRepository,
				gav, localOnly, remoteOnly);

		if (getLogger().isDebugEnabled()) {
			getLogger().debug(
					"Created ArtifactStoreRequest request for "
	...
	protected Model getPom(Variant variant, Request request, Response response)
		...
		// TODO: enable only one section retrieval of POM, ie. only mailing
		// lists, or team members
		...
		ArtifactStoreRequest gavRequest = getResourceStoreRequest(request,
				false, false, repositoryId, groupId, artifactId, version, null,
				null, "pom");
		...
		try {
			...
			ArtifactStoreHelper helper = mavenRepository
					.getArtifactStoreHelper();
	...
	protected Object getContent(Variant variant, boolean redirectTo,
		...
		ArtifactStoreRequest gavRequest = getResourceStoreRequest(request,
				false, false, repositoryId, groupId, artifactId, version,
	...
	public Object upload(Context context, Request request, Response response,
		...
		final PomArtifactManager pomManager = new PomArtifactManager(
		...
		try {
			for (FileItem fi : files) {
				if (fi.isFormField()) {
					...
					// we either received all the GAVs as params, or we have a
					// POM to work with (file1)
					boolean isPom = fi.getName().endsWith(".pom")
							|| fi.getName().endsWith("pom.xml");
					InputStream is = null;
					...
					if (uploadContext.isPomAvailable()) {
						if (isPom) {
							...
							// let it "thru" the pomManager to be able to get
							// GAV from it on later pass
						...
						try {
							// parse and read GAVs from stored POM, fill in them
							// all into context
							final ArtifactCoordinate coords = pomManager
									.getArtifactCoordinateFromTempPomFile();
							...
							getLogger()
									.info("Error occurred while reading the POM file. Malformed POM?",
											e);
							throw new ResourceException(
									Status.CLIENT_ERROR_BAD_REQUEST,
						...
						if (isPom) {
							...
							gavRequest = getResourceStoreRequest(request, true,
									false, uploadContext.getRepositoryId(),
									uploadContext.getGroupId(),
									uploadContext.getArtifactId(),
									uploadContext.getVersion(),
									uploadContext.getPackaging(), null, null);
						} else {
							gavRequest = getResourceStoreRequest(request, true,
									false, uploadContext.getRepositoryId(),
									uploadContext.getGroupId(),
					...
					final ArtifactStoreHelper helper = mr
							.getArtifactStoreHelper();

					// temporarily we disable SNAPSHOT upload
					// check is it a Snapshot repo
					if (RepositoryPolicy.SNAPSHOT.equals(mr
							.getRepositoryPolicy())) {
						getLogger()
					...
					if (!versionMatchesPolicy(gavRequest.getVersion(),
							mr.getRepositoryPolicy())) {
						getLogger()
								.info("Artifact version {} and {} Repository Policy {} mismatch",
										gavRequest.getVersion(), mr,
										mr.getRepositoryPolicy());
						throw new ResourceException(
								Status.CLIENT_ERROR_BAD_REQUEST,
								"The version "
					...
					if (isPom) {
						...
						if (uploadContext.isPomAvailable()) {
							...
							helper.storeArtifactWithGeneratedPom(gavRequest,
									uploadContext.getPackaging(), is, null);
						}
					}
				}
			}
	...
	protected boolean versionMatchesPolicy(String version,
			RepositoryPolicy policy) {
		boolean result = false;

		if ((RepositoryPolicy.SNAPSHOT.equals(policy) && Gav
				.isSnapshot(version))
				|| (RepositoryPolicy.RELEASE.equals(policy) && !Gav
						.isSnapshot(version))) {
			result = true;
		}
