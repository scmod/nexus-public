...
public class RepositoryPredefinedMirrorListPlexusResource extends
		AbstractRepositoryMirrorPlexusResource {
	public static final String RESOURCE_URI = "/repository_predefined_mirrors/{"
			+ REPOSITORY_ID_KEY + "}";
	...
	public RepositoryPredefinedMirrorListPlexusResource(
			final NexusRepositoryMetadataHandler repoMetadata) {
	...
	/**
	 * Get the predefined list of mirrors for a repository (as defined in the
	 * repository metadata). These mirrors may NOT be enabled, simply listed as
	 * available.
	 *
	 * @param repositoryId
	 *            The repository to retrieve the predefined mirrors for.
	 */
	@Override
	@GET
	public Object get(Context context, Request request, Response response,
		...
		if (metadata != null) {
			for (RepositoryMirrorMetadata mirror : (List<RepositoryMirrorMetadata>) metadata
					.getMirrors()) {
	...
	private RepositoryMetadata getMetadata(String repositoryId)
		...
		try {
			...
			throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST,
					"Invalid repository ID", e);
		} catch (MetadataHandlerException e) {
			getLogger().info(
					"Unable to retrieve metadata, returning no items: "
							+ e.getMessage());
		} catch (IOException e) {
			getLogger().error("Unable to retrieve metadata", e);
			throw new ResourceException(Status.SERVER_ERROR_INTERNAL,
					"Metadata handling error", e);
		}
