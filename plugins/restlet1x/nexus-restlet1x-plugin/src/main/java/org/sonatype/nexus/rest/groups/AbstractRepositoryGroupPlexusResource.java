...
public abstract class AbstractRepositoryGroupPlexusResource extends
	...
	protected RepositoryGroupResource buildGroupResource(Request request,
			String groupId) throws NoSuchRepositoryException, ResourceException {
		...
		if (repo.getRepositoryKind().isFacetAvailable(GroupRepository.class)) {
			return buildGroupResource(request,
					repo.adaptToFacet(GroupRepository.class));
		}
	...
	protected RepositoryGroupResource buildGroupResource(Request request,
			GroupRepository group) throws ResourceException {
		RepositoryGroupResource resource = new RepositoryGroupResource();

		resource.setContentResourceURI(getRepositoryURLBuilder()
				.getExposedRepositoryContentUrl(group));
		...
		// just to trigger list creation, and not stay null coz of XStream
		// serialization
		...
		for (String repoId : group.getMemberRepositoryIds()) {
			...
			try {
				// NOTE: we must hit the registry each time and NOT call
				// groupRepo.getMemberRepositories, that doesn't block access
				member.setName(getRepositoryRegistry().getRepository(repoId)
						.getName());
			} catch (NoSuchRepositoryAccessException e) {
				// access denied 403
				getLogger()
						.debug("Blocking access to repository group, based on permissions.");

				throw new ResourceException(Status.CLIENT_ERROR_FORBIDDEN);
			} catch (NoSuchRepositoryException e) {
				getLogger().debug(
						"Found missing repo id: " + repoId
								+ " contained in group");
			}

			member.setResourceURI(createChildReference(request, this, repoId)
					.toString());
	...
	protected void createOrUpdateRepositoryGroup(RepositoryGroupResource model,
	...
	protected void updateRepositoryGroup(RepositoryGroupResource model)
		...
		try {
			GroupRepository group = getRepositoryRegistry()
					.getRepositoryWithFacet(model.getId(),
			...
			// access denied 403
			getLogger().warn(
					"Repository referenced by Repository Group Access Eenied, ID="
							+ model.getId(), e);
			...
			getLogger().warn(
					"Repository referenced by Repository Group Not Found, ID="
							+ model.getId(), e);
			...
			getLogger().warn(
					"Invalid grouping detected!, GroupId=" + model.getId(), e);

			throw new PlexusResourceException(Status.CLIENT_ERROR_BAD_REQUEST,
					"Invalid grouping requested, GroupId=" + model.getId(), e,
	...
	protected void createRepositoryGroup(RepositoryGroupResource model)
			throws ResourceException {
		try {
			ContentClass contentClass = repositoryTypeRegistry
					.getRepositoryContentClass(GroupRepository.class,
							model.getProvider());

			RepositoryTemplate template = (RepositoryTemplate) getRepositoryTemplates()
					.getTemplates(GroupRepository.class, contentClass,
							new ProviderHint(model.getProvider())).pick();
			...
			template.getConfigurableRepository().setLocalStatus(
					LocalStatus.IN_SERVICE);

			// we create an empty group
			GroupRepository groupRepository = (GroupRepository) template
					.create();

			ArrayList<String> memberIds = new ArrayList<String>(model
		...
		/*
		 * catch ( NoSuchRepositoryAccessException e ) { // access denied 403
		 * getLogger().warn(
		 * "Repository referenced by Repository Group Access Denied, ID=" +
		 * model.getId(), e ); throw new PlexusResourceException(
		 * Status.CLIENT_ERROR_BAD_REQUEST,
		 * "Repository referenced by Repository Group Access Denied, GroupId=" +
		 * model.getId(), e, getNexusErrorResponse( "repositories",
		 * "Repository referenced by Repository Group Access Denied" ) ); }
		 */
		catch (NoSuchRepositoryException e) {
			getLogger().warn(
