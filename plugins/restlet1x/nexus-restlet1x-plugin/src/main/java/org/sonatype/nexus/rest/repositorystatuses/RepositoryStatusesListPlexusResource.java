...
public class RepositoryStatusesListPlexusResource extends
	...
	private final boolean adminOnly = SystemPropertiesHelper.getBoolean(
			ADMIN_ONLY_KEY, false);
	...
	/**
	 * Get the list of all repository statuses. The remote statuses in case of
	 * Proxy repositories are cached (to avoid network flooding). You can force
	 * the remote status recheck by adding the "forceCheck" query parameter, but
	 * be aware, that this one inbound REST Request will induce as many Nexus
	 * outbound requests as many proxy repositories you have defined.
	 *
	 * @param forceCheck
	 *            If true, will force a remote check of status (Optional).
	 */
	@Override
	@GET
	public Object get(Context context, Request request, Response response,
		...
		for (Repository repository : repositories) {
			...
			repoRes.setResourceURI(createChildReference(request, this,
			...
			repoRes.getStatus().setLocalStatus(
					repository.getLocalStatus().toString());
