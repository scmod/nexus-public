 ...
 * generate and email a user password to the user.
 *
 * @author tstevens
 */
@Singleton
...
public class UserForgotPasswordPlexusResource extends
		AbstractUserPlexusResource {
	...
	public PathProtectionDescriptor getResourceProtection() {
		return new PathProtectionDescriptor(getResourceUri(),
				"authcBasic,perms[security:usersforgotpw]");
	}
	...
	public Object post(Context context, Request request, Response response,
		...
		if (forgotPasswordRequest != null) {
			UserForgotPasswordResource resource = forgotPasswordRequest
					.getData();

			try {
				if (!isAnonymousUser(resource.getUserId(), request)) {
					getSecuritySystem().forgotPassword(resource.getUserId(),
							resource.getEmail());

					response.setStatus(Status.SUCCESS_ACCEPTED);
				} else {
					response.setStatus(Status.CLIENT_ERROR_BAD_REQUEST,
							"Anonymous user cannot forget password");

					getLogger().debug(
							"Anonymous user forgot password is blocked");
				}
