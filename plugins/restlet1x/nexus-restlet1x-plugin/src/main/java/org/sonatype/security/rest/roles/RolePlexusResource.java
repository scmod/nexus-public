...
public class RolePlexusResource extends AbstractRolePlexusResource {
	...
	public PathProtectionDescriptor getResourceProtection() {
		return new PathProtectionDescriptor("/roles/*",
	...
	public Object get(Context context, Request request, Response response,
		...
		try {
			AuthorizationManager authzManager = getSecuritySystem()
					.getAuthorizationManager(ROLE_SOURCE);
			result.setData(securityToRestModel(
					authzManager.getRole(getRoleId(request)), request, false));
			...
			this.getLogger().warn(
					"Could not found AuthorizationManager: " + ROLE_SOURCE, e);
			// we should not ever get here
			throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST,
					"Authorization Manager for: " + ROLE_SOURCE
							+ " could not be found.");
		}
		return result;
	}
	...
	public Object put(Context context, Request request, Response response,
		...
		if (resourceRequest != null) {
			...
			try {
				AuthorizationManager authzManager = getSecuritySystem()
						.getAuthorizationManager(ROLE_SOURCE);
				Role role = restToSecurityModel(
						authzManager.getRole(resource.getId()), resource);
				...
				resourceResponse.getData().setResourceURI(
						createChildReference(request, resource.getId())
								.toString());
				...
				this.getLogger().warn(
						"Could not found AuthorizationManager: " + ROLE_SOURCE,
						e);
				// we should not ever get here
				throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST,
						"Authorization Manager for: " + ROLE_SOURCE
