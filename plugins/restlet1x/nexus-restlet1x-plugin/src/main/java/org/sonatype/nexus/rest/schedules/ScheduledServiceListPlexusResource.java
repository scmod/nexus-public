...
public class ScheduledServiceListPlexusResource extends
		AbstractScheduledServicePlexusResource {
	 ...
	 *
	 * @param allTasks
	 *            If true, will return all tasks, even non-exposed tasks.
	 */
	@Override
	@GET
	public Object get(Context context, Request request, Response response,
		...
		Map<String, List<ScheduledTask<?>>> tasksMap = getNexusScheduler()
				.getAllTasks();
		...
		for (String key : tasksMap.keySet()) {
			...
			for (ScheduledTask<?> task : tasks) {
				...
				if (allTasks || isExposed) {
					...
					item.setResourceURI(createChildReference(request, this,
							task.getId()).toString());
					...
					ScheduledTaskDescriptor descriptor = getNexusConfiguration()
							.getScheduledTaskDescriptor(task.getType());
					if (descriptor != null) {
						item.setTypeName(descriptor.getName());
					}
					item.setCreated(task.getScheduledAt() == null ? "n/a"
							: task.getScheduledAt().toString());
					item.setLastRunTime(task.getLastRun() == null ? "n/a"
							: task.getLastRun().toString());
					final Date nextRunTime = getNextRunTime(task);
					item.setNextRunTime(nextRunTime == null ? "n/a"
	...
	public Object post(Context context, Request request, Response response,
		...
		if (serviceRequest != null) {
			ScheduledServiceBaseResource serviceResource = serviceRequest
					.getData();
			try {
				Schedule schedule = getModelSchedule(serviceRequest.getData());
				ScheduledTask<?> task = null;

				final NexusTask<?> nexusTask = getModelNexusTask(
						serviceResource, request);

				if (getLogger().isDebugEnabled()) {
					getLogger().debug(
							"Creating task with type '" + nexusTask.getClass()
									+ "': " + nexusTask.getName() + " ("
									+ nexusTask.getId() + ")");
				}
				...
				if (schedule != null) {
					task = getNexusScheduler().schedule(
							getModelName(serviceResource), nexusTask, schedule);
				} else {
					task = getNexusScheduler().schedule(
							getModelName(serviceResource), nexusTask,
				...
				// Just need to update the id, as the incoming data is a POST w/
				// no id
				resourceStatus.getResource().setId(task.getId());
				resourceStatus.setResourceURI(createChildReference(request,
						this, task.getId()).toString());
				resourceStatus.setStatus(task.getTaskState().toString());
				resourceStatus.setReadableStatus(getReadableState(task
						.getTaskState()));
				resourceStatus.setCreated(task.getScheduledAt() == null ? "n/a"
						: task.getScheduledAt().toString());
				resourceStatus.setLastRunResult(TaskState.BROKEN.equals(task
						.getTaskState()) ? "Error" : "Ok");
				resourceStatus.setLastRunTime(task.getLastRun() == null ? "n/a"
						: task.getLastRun().toString());
				resourceStatus.setNextRunTime(task.getNextRun() == null ? "n/a"
						: task.getNextRun().toString());
				if (task.getScheduledAt() != null) {
					resourceStatus.setCreatedInMillis(task.getScheduledAt()
							.getTime());
				}
				if (task.getLastRun() != null) {
					resourceStatus.setLastRunTimeInMillis(task.getLastRun()
							.getTime());
				}
				if (task.getNextRun() != null) {
					resourceStatus.setNextRunTimeInMillis(task.getNextRun()
							.getTime());
				}
				...
				getLogger().warn(
						"Execution of task " + getModelName(serviceResource)
								+ " rejected.");
