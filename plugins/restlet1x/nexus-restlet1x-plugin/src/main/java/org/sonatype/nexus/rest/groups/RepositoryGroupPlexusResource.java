...
public class RepositoryGroupPlexusResource extends
		AbstractRepositoryGroupPlexusResource {
	...
	public PathProtectionDescriptor getResourceProtection() {
		return new PathProtectionDescriptor("/repo_groups/*",
				"authcBasic,perms[nexus:repogroups]");
	}
	...
	public Object get(Context context, Request request, Response response,
		...
		try {
			groupRepo = getRepositoryRegistry().getRepositoryWithFacet(
					getGroupId(request), GroupRepository.class);
			result.setData(buildGroupResource(request, groupRepo));
		} catch (NoSuchRepositoryAccessException e) {
			// access denied 403
			getLogger()
					.debug("Blocking access to all repository groups, based on permissions.");

			throw new ResourceException(Status.CLIENT_ERROR_FORBIDDEN,
					"Access Denied to Repository Group");
		} catch (NoSuchRepositoryException e) {
			getLogger().warn(
					"Repository Group not found, id=" + getGroupId(request));
			...
			if (StringUtils.isEmpty(resource.getId())) {
				...
				throw new PlexusResourceException(
						...
						getNexusErrorResponse("repositories",
								"Repository group id can't be empty! "));
			}

			createOrUpdateRepositoryGroup(resource, false);

			try {
				result.setData(buildGroupResource(request, groupRequest
						.getData().getId()));
			} catch (NoSuchRepositoryException e) {
				throw new PlexusResourceException(
						Status.CLIENT_ERROR_NOT_FOUND,
						...
						getNexusErrorResponse("repositories",
								"Repository group id is invalid! "));
			}
		}
	...
	public void delete(Context context, Request request, Response response)
		...
		try {
			...
			getRepositoryRegistry().getRepositoryWithFacet(getGroupId(request),
					GroupRepository.class);

			getNexusConfiguration().deleteRepository(getGroupId(request));
		} catch (NoSuchRepositoryAccessException e) {
			getLogger()
					.warn("Repository group Access Denied, id="
							+ getGroupId(request));
			...
			getLogger().warn(
					"Repository group cannot be deleted, it has dependants, id="
							+ getGroupId(request));

			throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST,
					"Repository Group Cannot be deleted");
		} catch (IOException e) {
			...
			getLogger().warn(
					"Not allowed to delete Repository Group '"
							+ getGroupId(request) + "'", e);
