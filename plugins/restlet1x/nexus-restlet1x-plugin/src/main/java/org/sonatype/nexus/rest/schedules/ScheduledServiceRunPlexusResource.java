...
public class ScheduledServiceRunPlexusResource extends
	...
	public PathProtectionDescriptor getResourceProtection() {
		return new PathProtectionDescriptor("/schedule_run/*",
				"authcBasic,perms[nexus:tasksrun]");
	}

	/**
	 * Run the specified scheduled task right now. Will then be rescheduled upon
	 * completion for normal run.
	 *
	...
	public Object get(Context context, Request request, Response response,
		...
		try {
			ScheduledTask<?> task = getNexusScheduler().getTaskById(
					scheduledServiceId);
			...
			if (resource != null) {
				...
				throw new ResourceException(Status.CLIENT_ERROR_NOT_FOUND,
						"Invalid schedule id (" + scheduledServiceId
								+ "), can't load task.");
			}
			return result;
		} catch (NoSuchTaskException e) {
			throw new ResourceException(Status.CLIENT_ERROR_NOT_FOUND,
					"There is no task with ID=" + scheduledServiceId);
		}
	}
	...
	public void delete(Context context, Request request, Response response)
		...
		try {
			getNexusScheduler().getTaskById(getScheduledServiceId(request))
					.cancelOnly();

			response.setStatus(Status.SUCCESS_NO_CONTENT);
		} catch (NoSuchTaskException e) {
			response.setStatus(Status.CLIENT_ERROR_NOT_FOUND,
					"Scheduled service not found!");
		}
	}
