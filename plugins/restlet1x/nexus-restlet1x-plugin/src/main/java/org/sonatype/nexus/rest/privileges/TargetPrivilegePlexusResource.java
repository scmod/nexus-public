...
public class TargetPrivilegePlexusResource extends
	...
	/**
	 * Add a new set of target privileges against a repository (or group) in
	 * nexus.
	 */
	@Override
	@POST
	public Object post(Context context, Request request, Response response,
		...
		if (resourceRequest != null) {
			...
			// currently we are allowing only of repotarget privs, so enforcing
			// checkfor it
			if (!TargetPrivilegeDescriptor.TYPE.equals(resource.getType())) {
				throw new PlexusResourceException(
						Status.CLIENT_ERROR_BAD_REQUEST,
						"Configuration error.", getErrorResponse("type",
								"Not allowed privilege type!"));
			}
			...
			if (methods == null || methods.size() == 0) {
				...
				try {
					...
					for (String method : methods) {
						...
						priv.setName(resource.getName() != null ? resource
								.getName() + " - (" + method + ")" : null);
						...
						priv.addProperty(
								...
								resource.getRepositoryTargetId());
						...
						priv.addProperty(
								...
								resource.getRepositoryGroupId());

						priv = getSecuritySystem().getAuthorizationManager(
								DEFAULT_SOURCE).addPrivilege(priv);
					...
					this.getLogger().warn(
							"Could not find the default AuthorizationManager",
							e);
