...
public class RepositoryRouteListPlexusResource extends
	...
	public Object get(Context context, Request request, Response response,
		...
		Map<String, RepositoryPathMapping> mappings = getRepositoryMapper()
				.getMappings();
		...
		for (RepositoryPathMapping item : mappings.values()) {
			...
			if (!item.getGroupId().equals("*")) {
				...
				try {
					this.getRepositoryRegistry().getRepositoryWithFacet(
							item.getGroupId(), GroupRepository.class);
				} catch (NoSuchRepositoryAccessException e) {
					getLogger().debug(
							"Access Denied to Group '" + item.getGroupId()
									+ "' contained within route: + '"
									+ item.getId() + "'!", e);
					continue;
				} catch (NoSuchRepositoryException e) {
					getLogger().warn(
							"Cannot find group '" + item.getGroupId()
									+ "' declared within route: + '"
									+ item.getId() + "'!", e);
					continue;
				}
			}
			...
			resource.setResourceURI(createChildReference(request, this,
					item.getId()).toString());
			...
			try {
				resource.setRepositories(getRepositoryRouteMemberRepositoryList(
						request.getResourceRef(), item.getMappedRepositories(),
						request, item.getId()));
			} catch (NoSuchRepositoryAccessException e) {
				getLogger().debug(
						"Access Denied to Group '" + item.getGroupId()
	...
	public Object post(Context context, Request request, Response response,
		...
		if (routeRequest != null) {
			...
			if (!RepositoryRouteResource.BLOCKING_RULE_TYPE.equals(resource
					.getRuleType())
					&& (resource.getRepositories() == null || resource
			...
			try {
				...
				RepositoryPathMapping route = new RepositoryPathMapping(
						resource.getId(),
