...
public abstract class AbstractRepositoryPlexusResource extends
	...
	@Inject
	public void setRepositoryURLBuilder(
	...
	// CLEAN
	public String getRestRepoRemoteStatus(ProxyRepository repository,
			Request request, Response response) throws ResourceException {
		Form form = request.getResourceRef().getQueryAsForm();

		boolean forceCheck = form.getFirst("forceCheck") != null;

		RemoteStatus rs = repository.getRemoteStatus(new ResourceStoreRequest(
				RepositoryItemUid.PATH_ROOT), forceCheck);
		...
		return rs == null ? null : rs.toString()
				+ (rs.getReason() == null ? "" : ":" + rs.getReason());
	}
	...
	public String getRestRepoType(Repository repository) {
		if (repository.getRepositoryKind().isFacetAvailable(
		...
		} else if (repository.getRepositoryKind().isFacetAvailable(
			...
			throw new IllegalArgumentException("The passed model with class"
					+ repository.getClass().getName() + " is not recognized!");
		}
	}

	protected RepositoryListResourceResponse listRepositories(Request request,
			boolean allReposes) throws ResourceException {
		return listRepositories(request, allReposes, true);
	}

	// clean
	protected RepositoryListResourceResponse listRepositories(Request request,
			boolean allReposes, boolean includeGroups) throws ResourceException {
		RepositoryListResourceResponse result = new RepositoryListResourceResponse();

		RepositoryListResource repoRes;

		Collection<Repository> repositories = getRepositoryRegistry()
				.getRepositories();

		for (Repository repository : repositories) {
			// To save UI changes at the moment, not including groups in repo
			// call
			if ((allReposes || repository.isUserManaged())
					&& (includeGroups || !repository.getRepositoryKind()
							.isFacetAvailable(GroupRepository.class))) {
				repoRes = new RepositoryListResource();

				repoRes.setResourceURI(createRepositoryReference(request,
						repository.getId()).toString());

				repoRes.setContentResourceURI(repositoryURLBuilder
						.getExposedRepositoryContentUrl(repository));

				repoRes.setRepoType(getRestRepoType(repository));

				repoRes.setProvider(NexusCompat
						.getRepositoryProviderHint(repository));

				repoRes.setProviderRole(NexusCompat
						.getRepositoryProviderRole(repository));

				repoRes.setFormat(repository.getRepositoryContentClass()
						.getId());
				...
				if (repository.getRepositoryKind().isFacetAvailable(
					...
					repoRes.setRepoPolicy(repository
				...
				if (repository.getRepositoryKind().isFacetAvailable(
						ProxyRepository.class)) {
					repoRes.setRemoteUri(repository.adaptToFacet(
							ProxyRepository.class).getRemoteUrl());
				}
	...
	// clean
	protected RepositoryResourceResponse getRepositoryResourceResponse(
			Request request, String repoId) throws ResourceException {
	 ...
	 */
	public RepositoryBaseResource getRepositoryRestModel(Request request,
		...
		if (repository.getRepositoryKind().isFacetAvailable(
			...
			resource = getRepositoryProxyRestModel(repository
					.adaptToFacet(ProxyRepository.class));
		} else if (repository.getRepositoryKind().isFacetAvailable(
				ShadowRepository.class)) {
			return getRepositoryShadowRestModel(request,
					repository.adaptToFacet(ShadowRepository.class));
		} else {
			resource = new RepositoryResource();
		}

		resource.setContentResourceURI(repositoryURLBuilder
				.getExposedRepositoryContentUrl(repository));

		resource.setProvider(NexusCompat.getRepositoryProviderHint(repository));

		resource.setProviderRole(NexusCompat
				.getRepositoryProviderRole(repository));
		...
		// TODO: remove the default local storage, this is a work around for
		// NEXUS-1994
		// the new 1.4 API doesn't store the default URL, well, it is part of
		// the CRepo, but it is not exposed.
		// so we can figure it out again, I think the default local Storage
		// should be removed from the REST message
		// which is part of the reason for not exposing it. The other part is it
		// is not used anywhere except to set
		...
		String defaultLocalStorageUrl = ((AbstractRepository) repository)
				.getCurrentCoreConfiguration().getConfiguration(false).defaultLocalStorageUrl;
		resource.setDefaultLocalStorageUrl(defaultLocalStorageUrl);

		// if not user set (but using default), this is null, otherwise it
		// contains user-set value
		String overrideLocalStorageUrl = ((AbstractRepository) repository)
				.getCurrentCoreConfiguration().getConfiguration(false)
		...
		if (repository.getRepositoryKind().isFacetAvailable(
				MavenRepository.class)) {
			resource.setRepoPolicy(repository
					.adaptToFacet(MavenRepository.class).getRepositoryPolicy()
					.toString());

			if (repository.getRepositoryKind().isFacetAvailable(
					MavenProxyRepository.class)) {
				resource.setChecksumPolicy(repository
						.adaptToFacet(MavenProxyRepository.class)
						.getChecksumPolicy().toString());

				resource.setDownloadRemoteIndexes(repository.adaptToFacet(
						MavenProxyRepository.class).isDownloadRemoteIndexes());
			}
		}
		// as this is a required field on ui, we need this to be set for
		// non-maven type repos
		else {
	 ...
	 * Converting App model to REST DTO.
	 */
	public RepositoryProxyResource getRepositoryProxyRestModel(
		...
		if (repository.getRepositoryKind().isFacetAvailable(
				MavenProxyRepository.class)) {
			resource.setArtifactMaxAge(repository.adaptToFacet(
					MavenProxyRepository.class).getArtifactMaxAge());

			resource.setMetadataMaxAge(repository.adaptToFacet(
					MavenProxyRepository.class).getMetadataMaxAge());

			resource.setItemMaxAge(repository.adaptToFacet(
					MavenProxyRepository.class).getItemMaxAge());
		} else {
			// This is a total hack to be able to retrieve this data from a non
			// core repo if available
			try {

				// NXCM-5131 Ask for itemMaxAge first, because it's already
				// introduced in AbstractProxyRepository and
				// may be a superclass for non-maven repositories (e.g. NuGet)
				Method itemMethod = repository.getClass().getMethod(
						"getItemMaxAge", new Class<?>[0]);
				if (itemMethod != null) {
					resource.setItemMaxAge((Integer) itemMethod.invoke(
							repository, new Object[0]));
				}

				Method artifactMethod = repository.getClass().getMethod(
						"getArtifactMaxAge", new Class<?>[0]);
				if (artifactMethod != null) {
					resource.setArtifactMaxAge((Integer) artifactMethod.invoke(
							repository, new Object[0]));
				}

				Method metadataMethod = repository.getClass().getMethod(
						"getMetadataMaxAge", new Class<?>[0]);
				if (metadataMethod != null) {
					resource.setMetadataMaxAge((Integer) metadataMethod.invoke(
							repository, new Object[0]));
				}
			} catch (Exception e) {
	...
	public RepositoryShadowResource getRepositoryShadowRestModel(
		...
		resource.setContentResourceURI(repositoryURLBuilder
				.getExposedRepositoryContentUrl(shadow));
	...
	protected CRemoteConnectionSettings convertRemoteConnectionSettings(
		...
		cRemoteConnectionSettings.setConnectionTimeout(remoteConnectionSettings
				.getConnectionTimeout() * 1000);

		cRemoteConnectionSettings.setQueryString(remoteConnectionSettings
				.getQueryString());

		cRemoteConnectionSettings
				.setRetrievalRetryCount(remoteConnectionSettings
						.getRetrievalRetryCount());

		cRemoteConnectionSettings
				.setUserAgentCustomizationString(remoteConnectionSettings
						.getUserAgentString());
