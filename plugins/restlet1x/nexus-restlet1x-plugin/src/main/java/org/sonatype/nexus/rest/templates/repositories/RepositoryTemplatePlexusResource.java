...
public class RepositoryTemplatePlexusResource extends
	...
	public PathProtectionDescriptor getResourceProtection() {
		return new PathProtectionDescriptor("/templates/repositories/*",
				"authcBasic,perms[nexus:repotemplates]");
	}
	 ...
	 *
	 * @param repositoryId
	 *            The repository template to access.
	 */
	@Override
	@GET
	public Object get(Context context, Request request, Response response,
		...
		try {
			...
			if (ProxyRepository.class.isAssignableFrom(template.getMainFacet())) {
			...
			} else if (HostedRepository.class.isAssignableFrom(template
					.getMainFacet())) {
				repoRes = createHosted(template);
			} else if (ShadowRepository.class.isAssignableFrom(template
					.getMainFacet())) {
				repoRes = createShadow(template);
			} else if (GroupRepository.class.isAssignableFrom(template
					.getMainFacet())) {
				...
				throw new ResourceException(Status.SERVER_ERROR_INTERNAL,
						"Unrecognized repository template with ID='"
								+ template.getId() + "' and mainFacet='"
								+ template.getMainFacet().getName() + "'!");
			}
	...
	private RepositoryBaseResource createShadow(RepositoryTemplate template) {
		...
		AbstractShadowRepositoryConfiguration cfg = (AbstractShadowRepositoryConfiguration) template
				.getConfigurableRepository().getCurrentCoreConfiguration()
	...
	private RepositoryBaseResource createProxy(RepositoryTemplate template) {
		...
		AbstractMavenRepositoryConfiguration repoCfg = (AbstractMavenRepositoryConfiguration) template
				.getConfigurableRepository().getCurrentCoreConfiguration()
				.getExternalConfiguration().getConfiguration(false);
