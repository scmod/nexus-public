	...
	public boolean applies(RoleAndPrivilegeListResource resource) {
		if (resource != null) {
			if (resource.getType().equals("role")) {
				if (((isShowRoles() && !resource.isExternal() && !(getUserId() != null && getRoleIds()
						.isEmpty())) || (isShowExternalRoles() && resource
						.isExternal()))
						&& (!getHiddenRoleIds().contains(resource.getId()))
						&& (resource.isExternal() || (((getRoleIds().isEmpty() && !isOnlySelected()) || getRoleIds()
								.contains(resource.getId()))))
						&& (StringUtils.isEmpty(getText()) || Pattern
								.compile(Pattern.quote(getText()),
										Pattern.CASE_INSENSITIVE)
								.matcher(resource.getName()).find())) {
					return true;
				}
			} else if (resource.getType().equals("privilege")) {
				if (isShowPrivileges()
						&& (!getHiddenPrivilegeIds().contains(resource.getId()))
						&& ((getPrivilegeIds().isEmpty() && !isOnlySelected()) || getPrivilegeIds()
								.contains(resource.getId()))
						&& (StringUtils.isEmpty(getText()) || Pattern
								.compile(Pattern.quote(getText()),
										Pattern.CASE_INSENSITIVE)
