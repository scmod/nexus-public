...
public class RepositoryStatusPlexusResource extends
	...
	public PathProtectionDescriptor getResourceProtection() {
		return new PathProtectionDescriptor("/repositories/*/status",
				"authcBasic,perms[nexus:repostatus]");
	}
	...
	public Object get(Context context, Request request, Response response,
		...
		try {
			...
			if (repo.getRepositoryKind()
					.isFacetAvailable(ProxyRepository.class)) {
				ProxyRepository prepo = repo
						.adaptToFacet(ProxyRepository.class);

				resource.setRemoteStatus(getRestRepoRemoteStatus(prepo,
						request, response));
	...
	public Object put(Context context, Request request, Response response,
		...
		if (repoStatusRequest != null) {
			try {
				...
				if (resource.getLocalStatus() == null) {
					throw new ResourceException(
							...
							"Local status must be defined");
				}

				LocalStatus localStatus = EnumUtil.valueOf(
						resource.getLocalStatus(), LocalStatus.class);
				if (RepositoryBaseResourceConverter.REPO_TYPE_VIRTUAL
					...
					result = (RepositoryStatusResourceResponse) this.get(
							context, request, response, null);
				} else {
					...
					if (repository.getRepositoryKind().isFacetAvailable(
						...
						ProxyMode proxyMode = EnumUtil.valueOf(
								resource.getProxyMode(), ProxyMode.class);
						repository.adaptToFacet(ProxyRepository.class)
								.setProxyMode(proxyMode);
					}

					// update dependant shadows too
					for (ShadowRepository shadow : getRepositoryRegistry()
							.getRepositoriesWithFacet(ShadowRepository.class)) {
						if (repository.getId().equals(
								shadow.getMasterRepository().getId())) {
							shadow.setLocalStatus(localStatus);
						}
					}
					...
					for (ShadowRepository shadow : getRepositoryRegistry()
						...
						if (repository.getId().equals(
							...
							dependent.setFormat(shadow
									.getRepositoryContentClass().getId());

							dependent.setLocalStatus(shadow.getLocalStatus()
									.toString());
			...
			// {
			// getLogger().warn( "Configuration unacceptable, repoId=" + repoId,
			// e );
			//
