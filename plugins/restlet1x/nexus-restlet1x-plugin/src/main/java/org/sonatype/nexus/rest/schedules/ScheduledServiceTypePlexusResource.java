...
public class ScheduledServiceTypePlexusResource extends
	...
	public PathProtectionDescriptor getResourceProtection() {
		return new PathProtectionDescriptor(getResourceUri(),
				"authcBasic,perms[nexus:tasktypes]");
	}

	/**
	 * Get the list of scheduled service types available in nexus. And all of
	 * the configuration parameters available for each type.
	 */
	@Override
	@GET
	public Object get(Context context, Request request, Response response,
		...
		List<ScheduledTaskDescriptor> taskDescriptors = getNexusConfiguration()
				.listScheduledTaskDescriptors();
		...
		for (ScheduledTaskDescriptor taskDescriptor : taskDescriptors) {
			if (taskDescriptor.isExposed()) {
				...
				type.setFormFields((List<FormFieldResource>) formFieldToDTO(
						taskDescriptor.formFields(), FormFieldResource.class));
	...
	private void sortTaskType(List<ScheduledServiceTypeResource> types) {
		Collections.sort(types, new Comparator<ScheduledServiceTypeResource>() {
			public int compare(ScheduledServiceTypeResource t1,
					ScheduledServiceTypeResource t2) {
