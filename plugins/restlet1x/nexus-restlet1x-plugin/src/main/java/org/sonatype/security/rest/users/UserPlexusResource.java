...
public class UserPlexusResource extends AbstractUserPlexusResource {
	...
	public PathProtectionDescriptor getResourceProtection() {
		return new PathProtectionDescriptor("/users/*",
	...
	public Object get(Context context, Request request, Response response,
		...
		try {
			result.setData(securityToRestModel(
					getSecuritySystem().getUser(getUserId(request)), request,
					false));
	...
	public Object put(Context context, Request request, Response response,
		...
		if (resourceRequest != null) {
			...
			// the password can not be set on update, The only way to set a
			// password is using the users_setpw resource
			...
			try {
				User user = restToSecurityModel(
						getSecuritySystem().getUser(resource.getUserId()),
						resource);
				...
				result.getData().setResourceURI(
						createChildReference(request, resource.getUserId())
								.toString());
	...
	protected boolean isCurrentUser(Request request) {
		...
		if (subject == null || subject.getPrincipal() == null) {
			return false; // not the current user because there is no current
							// user
		}
