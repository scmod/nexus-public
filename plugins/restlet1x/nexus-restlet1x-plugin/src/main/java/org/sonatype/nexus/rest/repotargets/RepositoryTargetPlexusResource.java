...
public class RepositoryTargetPlexusResource extends
		AbstractRepositoryTargetPlexusResource {
	...
	public PathProtectionDescriptor getResourceProtection() {
		return new PathProtectionDescriptor("/repo_targets/*",
				"authcBasic,perms[nexus:targets]");
	}
	...
	public Object get(Context context, Request request, Response response,
		...
		Target target = getTargetRegistry().getRepositoryTarget(
				getRepoTargetId(request));
			...
			if (target != null) {
				if (validate(false, resource)) {
					try {
						...
						// TODO: fix because this happens before we validate, we
						...
						getLogger()
								.warn("Got IOException during creation of repository target!",
										e);
	...
	public void delete(Context context, Request request, Response response)
		...
		if (target != null) {
			try {
				getTargetRegistry().removeRepositoryTarget(
						getRepoTargetId(request));
