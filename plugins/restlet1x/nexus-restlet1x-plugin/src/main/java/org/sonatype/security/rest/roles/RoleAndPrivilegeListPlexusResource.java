...
public class RoleAndPrivilegeListPlexusResource extends
	...
	public PathProtectionDescriptor getResourceProtection() {
		return new PathProtectionDescriptor(getResourceUri(),
				"authcBasic,perms[security:roles]");
	}
	...
	public Object post(Context context, Request request, Response response,
		...
		try {
			...
			for (Role role : getSecuritySystem().getAuthorizationManager(
					DEFAULT_SOURCE).listRoles()) {
			...
			for (Privilege privilege : getSecuritySystem()
					.getAuthorizationManager(DEFAULT_SOURCE).listPrivileges()) {
			...
			if (!StringUtils.isEmpty(filterRequest.getData().getUserId())) {
				try {
					User user = getSecuritySystem().getUser(
							filterRequest.getData().getUserId());

					List<PlexusRoleResource> plexusRoles = securityToRestModel(
							user).getRoles();
					...
					getLogger()
							.warn("Unable to load user, and retrieve any external roles assigned",
									e);
				}
			}
			...
			this.getLogger().error(
					"Unable to find AuthorizationManager 'default'", e);
			throw new ResourceException(Status.SERVER_ERROR_INTERNAL,
					"Unable to find AuthorizationManager 'default'");
		}
	...
	protected RoleAndPrivilegeListResource toDTO(PlexusRoleResource roleResource) {
		...
		resource.setDescription("External role from the "
				+ roleResource.getSource()
	...
	protected List<RoleAndPrivilegeListResource> generateResultSet(
			List<RoleAndPrivilegeListResource> resources, Form form) {
	...
	protected void sortResultSet(List<RoleAndPrivilegeListResource> resources,
			String sort, String dir) {
		if (!StringUtils.isEmpty(sort)) {
			RoleAndPrivilegeListResourceComparator comparator = new RoleAndPrivilegeListResourceComparator(
					sort, dir);
			Collections.sort(resources, comparator);
		}
	}

	protected List<RoleAndPrivilegeListResource> paginateResultSet(
			List<RoleAndPrivilegeListResource> resources, String startStr,
			String limitStr) {
		int start;
		int limit;
