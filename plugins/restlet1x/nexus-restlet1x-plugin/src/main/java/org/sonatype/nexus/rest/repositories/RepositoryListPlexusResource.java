...
public class RepositoryListPlexusResource extends
	...
	public Object post(Context context, Request request, Response response,
		...
		if (repoRequest != null) {
			...
			try {
				...
				// UGLY HACK
				// This is all broken here, the conversions that happens (Repo
				// REST DTO -> CRepo DTO -> Repo creation)
				// is simply damn too stupid.
				// All this should be removed, and do not use C* config classes
				// anymore in REST API (see NEXUS-2505).
				// For now, this is a "backdoor", using manual template when we
				// have a CRepo object.
				ManuallyConfiguredRepositoryTemplate template = repositoryTemplateProvider
						.createManuallyTemplate(new CRepositoryCoreConfiguration(
								repositoryTemplateProvider
										.getApplicationConfiguration(), config,
								null));
	...
	/**
	 * Converting REST DTO + possible App model to App model. If app model is
	 * given, "update" happens, otherwise if target is null, "create".
	 *
	 * @return app model, merged or created
	 */
	public CRepository getRepositoryAppModel(RepositoryBaseResource resource,
			CRepository target) throws ResourceException {
		...
		if (target != null) {
			...
			ex = new Xpp3Dom(
					DefaultCRepository.EXTERNAL_CONFIGURATION_NODE_NAME);
		}
		...
		if (RepositoryBaseResourceConverter.REPO_TYPE_VIRTUAL.equals(resource
				.getRepoType())) {
		...
		} else if (!RepositoryBaseResourceConverter.REPO_TYPE_GROUP
			...
			exConf.setRepositoryPolicy(EnumUtil.valueOf(
					repoResource.getRepoPolicy(), RepositoryPolicy.class));

			if (repoResource.getOverrideLocalStorageUrl() != null) {
				appModel.setLocalStorage(new CLocalStorage());

				appModel.getLocalStorage().setUrl(
						repoResource.getOverrideLocalStorageUrl());
			...
			RepositoryResourceRemoteStorage remoteStorage = repoResource
					.getRemoteStorage();
			if (remoteStorage != null) {
				...
				appModel.getRemoteStorage().setUrl(
						remoteStorage.getRemoteStorageUrl());

				appModel.getRemoteStorage().setProvider(
						remoteProviderHintFactory
								.getDefaultRoleHint(remoteStorage
										.getRemoteStorageUrl()));
			}
		}
		...
		if (RepositoryProxyResource.class.isAssignableFrom(resource.getClass())) {
			appModel = getRepositoryProxyAppModel(
					(RepositoryProxyResource) resource, appModel);
		}
	 ...
	 * @return app model, merged or created
	 */
	public CRepository getRepositoryProxyAppModel(
			RepositoryProxyResource model, CRepository target)
			throws PlexusResourceException {
		M2RepositoryConfiguration exConf = new M2RepositoryConfiguration(
				(Xpp3Dom) target.getExternalConfiguration());

		if (model.getProvider().equals("maven2")
				|| model.getProvider().equals("maven1")) {
			exConf.setChecksumPolicy(EnumUtil.valueOf(
					model.getChecksumPolicy(), ChecksumPolicy.class));
		}
