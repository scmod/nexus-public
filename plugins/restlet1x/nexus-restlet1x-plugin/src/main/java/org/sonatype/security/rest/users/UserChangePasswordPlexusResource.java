...
@Named("UserChangePasswordPlexusResource")
@Produces({ "application/xml", "application/json" })
@Consumes({ "application/xml", "application/json" })
@Path(UserChangePasswordPlexusResource.RESOURCE_URI)
public class UserChangePasswordPlexusResource extends
	...
	public PathProtectionDescriptor getResourceProtection() {
		return new PathProtectionDescriptor(getResourceUri(),
				"authcBasic,perms[security:userschangepw]");
	}
	...
	public Object post(Context context, Request request, Response response,
		...
		if (changePasswordRequest != null) {
			UserChangePasswordResource resource = changePasswordRequest
					.getData();

			try {
				if (!isAnonymousUser(resource.getUserId(), request)) {
					getSecuritySystem().changePassword(resource.getUserId(),
					...
					getLogger().debug(
							"Anonymous user password change is blocked!");
				}
