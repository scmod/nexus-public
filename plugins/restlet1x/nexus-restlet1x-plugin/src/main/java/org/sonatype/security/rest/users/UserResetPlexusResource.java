...
/**
 * REST resource to reset a users password. Default implementations will
 * generate and email a user password to the user. <BR/>
 * This resource is similar to {@link UserForgotPasswordPlexusResource} except
 * that a system administrator can reset other users passwords.
 *
 * @author tstevens
 */
@Singleton
...
public class UserResetPlexusResource extends AbstractUserPlexusResource {
	...
	public PathProtectionDescriptor getResourceProtection() {
		return new PathProtectionDescriptor("/users_reset/*",
				"authcBasic,perms[security:usersreset]");
	}
		...
		try {
			if (!isAnonymousUser(userId, request)) {
				...
				throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST,
						"Anonymous user cannot reset password!");
			}
