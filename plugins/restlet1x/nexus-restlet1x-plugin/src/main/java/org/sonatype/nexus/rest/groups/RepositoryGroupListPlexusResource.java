...
public class RepositoryGroupListPlexusResource extends
	...
	public Object get(Context context, Request request, Response response,
		...
		Collection<GroupRepository> groups = getRepositoryRegistry()
		...
		try {
			for (GroupRepository group : groups) {
				...
				resource.setResourceURI(createRepositoryGroupReference(request,
				...
				resource.setFormat(getRepositoryRegistry()
						.getRepositoryWithFacet(group.getId(),
			...
			getLogger()
					.warn("Cannot find a repository group or repository declared within a group!",
							e);
