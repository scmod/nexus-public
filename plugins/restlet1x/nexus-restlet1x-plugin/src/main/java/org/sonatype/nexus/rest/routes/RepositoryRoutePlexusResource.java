...
public class RepositoryRoutePlexusResource extends
		AbstractRepositoryRoutePlexusResource {
	...
	public PathProtectionDescriptor getResourceProtection() {
		return new PathProtectionDescriptor("/repo_routes/*",
				"authcBasic,perms[nexus:routes]");
	}
	 ...
	 *
	 * @param routeId
	 *            The repository route to access.
	 */
	@Override
	@GET
	public Object get(Context context, Request request, Response response,
		...
		try {
			RepositoryPathMapping route = getRepositoryMapper().getMappings()
					.get(getRouteId(request));
			...
			if (!route.isAllGroups()) {
				...
				this.getRepositoryRegistry().getRepositoryWithFacet(
						route.getGroupId(), GroupRepository.class);
			}
			...
			getLogger()
					.debug("Access Denied to a repository declared within a group!",
							e);

			throw new ResourceException(Status.CLIENT_ERROR_FORBIDDEN, e);
		} catch (NoSuchRepositoryException e) {
			getLogger().warn(
					"Cannot find a repository declared within a group!", e);

			throw new ResourceException(Status.SERVER_ERROR_INTERNAL, e);
		} catch (IndexOutOfBoundsException e) {
			getLogger().warn(
					"Repository route not found, id=" + getRouteId(request));

			throw new ResourceException(Status.CLIENT_ERROR_NOT_FOUND,
					"Repository Route Not Found");
		}
	...
	public Object put(Context context, Request request, Response response,
		...
		if (routeRequest != null) {
			...
			if ((!RepositoryRouteResource.BLOCKING_RULE_TYPE.equals(resource
					.getRuleType()) && (resource.getRepositories() == null || resource
					.getRepositories().size() == 0))
					|| resource.getId() == null
					|| !resource.getId().equals(getRouteId(request))) {
				throw new PlexusResourceException(
						Status.CLIENT_ERROR_BAD_REQUEST,
						"The route cannot have zero repository members!",
						getNexusErrorResponse("repositories",
								"The route cannot have zero repository members!"));
			} else if (RepositoryRouteResource.BLOCKING_RULE_TYPE
					.equals(resource.getRuleType())) {
				resource.setRepositories(null);
			}

			try {
				RepositoryPathMapping route = getRepositoryMapper()
						.getMappings().get(getRouteId(request));
				...
				ArrayList<String> mappedReposes = new ArrayList<String>(
						resource.getRepositories().size());

				for (RepositoryRouteMemberRepository member : resource
						.getRepositories()) {
					mappedReposes.add(member.getId());
				}

				RepositoryPathMapping newRoute = new RepositoryPathMapping(
						route.getId(),
						resource2configType(resource.getRuleType()),
						resource.getGroupId(),
						Arrays.asList(new String[] { resource.getPattern() }),
						mappedReposes);
				...
				if (e.getCause() != null
					...
					throw new PlexusResourceException(
							Status.CLIENT_ERROR_BAD_REQUEST,
							"Configuration error.", getNexusErrorResponse(
									"pattern", e.getMessage()));
				} else {
					handleConfigurationException(e);
				}
			} catch (PatternSyntaxException e) {
				// TODO: fix because this happens before we validate, we need to
				// fix the validation.
				ErrorResponse errorResponse = getNexusErrorResponse("*",
						e.getMessage());
				throw new PlexusResourceException(
						Status.CLIENT_ERROR_BAD_REQUEST,
			 ...
			 * "Access Denied to a repository referenced within a route!", e );
			 * 
			 * throw new ResourceException( Status.CLIENT_ERROR_FORBIDDEN ); }
			 * catch ( NoSuchRepositoryException e ) { getLogger().warn(
			 * "Cannot find a repository referenced within a route!", e );
			 * 
			 * throw new PlexusResourceException(
			 * Status.CLIENT_ERROR_BAD_REQUEST,
