...
public class RepositoryPlexusResource extends AbstractRepositoryPlexusResource {
	...
	public PathProtectionDescriptor getResourceProtection() {
		return new PathProtectionDescriptor("/repositories/*",
	...
	public Object get(Context context, Request request, Response response,
		...
		return this.getRepositoryResourceResponse(request,
	...
	public Object put(Context context, Request request, Response response,
		...
		if (repoRequest != null) {
			try {
				...
				if (RepositoryBaseResourceConverter.REPO_TYPE_VIRTUAL
						.equals(resource.getRepoType())) {
					RepositoryShadowResource model = (RepositoryShadowResource) resource;

					try {
						ShadowRepository shadow = getRepositoryRegistry()
								.getRepositoryWithFacet(repoId,
						...
						shadow.setMasterRepository(getRepositoryRegistry()
								.getRepository(model.getShadowOf()));
						...
						getLogger().warn(
								"Repository access denied, id=" + repoId);
						...
						getLogger().warn(
								"Virtual repository not found, id=" + repoId);

						throw new ResourceException(
								Status.CLIENT_ERROR_NOT_FOUND,
								"Virtual repository Not Found");
					}
				} else {
					...
					try {
						...
						RepositoryWritePolicy writePolicy = (model
								.getWritePolicy() != null) ? RepositoryWritePolicy
								.valueOf(model.getWritePolicy())
						...
						if (repository.getRepositoryKind().isFacetAvailable(
							...
							if (proxyRepo.getRemoteAuthenticationSettings() != null
									&& UsernamePasswordRemoteAuthenticationSettings.class
											.isInstance(proxyRepo
													.getRemoteAuthenticationSettings())) {
								oldPasswordForRemoteStorage = ((UsernamePasswordRemoteAuthenticationSettings) proxyRepo
										.getRemoteAuthenticationSettings())
										.getPassword();
							}
							...
							if (remoteConnSettings != null) {
								proxyRepo
										.setRemoteConnectionSettings(remoteConnSettings);
							} else {
								proxyRepo.getRemoteStorageContext()
										.removeRemoteConnectionSettings();
							}

							// set auto block
							proxyRepo
									.setAutoBlockActive(((RepositoryProxyResource) model)
											.isAutoBlockActive());

							// set type validation
							proxyRepo
									.setFileTypeValidation(((RepositoryProxyResource) model)
						...
						if (repository.getRepositoryKind().isFacetAvailable(
							...
							RepositoryPolicy repoPolicy = EnumUtil.valueOf(
									model.getRepoPolicy(),
									RepositoryPolicy.class);
							repository.adaptToFacet(MavenRepository.class)
									.setRepositoryPolicy(repoPolicy);
							if (repository.getRepositoryKind()
									.isFacetAvailable(
											MavenProxyRepository.class)) {
								ChecksumPolicy checksum = EnumUtil.valueOf(
										model.getChecksumPolicy(),
										ChecksumPolicy.class);

								MavenProxyRepository pRepository = repository
										.adaptToFacet(MavenProxyRepository.class);
								pRepository.setChecksumPolicy(checksum);

								pRepository.setDownloadRemoteIndexes(model
										.isDownloadRemoteIndexes());

								pRepository.setChecksumPolicy(EnumUtil.valueOf(
										model.getChecksumPolicy(),
										ChecksumPolicy.class));
								...
								pRepository.setArtifactMaxAge(proxyModel
										.getArtifactMaxAge());

								pRepository.setMetadataMaxAge(proxyModel
										.getMetadataMaxAge());

								if (proxyModel.getItemMaxAge() != null) {
									pRepository.setItemMaxAge(proxyModel
											.getItemMaxAge());
								}
							}
						} else {
							// This is a total hack to be able to retrieve this
							// data from a non core repo if available
							try {
								RepositoryProxyResource proxyModel = (RepositoryProxyResource) model;

								// NXCM-5131 Ask for itemMaxAge first, because
								// it's already introduced in
								// AbstractProxyRepository and
								// may be a superclass for non-maven
								// repositories (e.g. NuGet)
								Method itemMethod = repository.getClass()
										.getMethod("setItemMaxAge", int.class);
								if (itemMethod != null
										&& proxyModel.getItemMaxAge() != null) {
									itemMethod.invoke(repository,
											proxyModel.getItemMaxAge());
								}

								Method artifactMethod = repository.getClass()
										.getMethod("setArtifactMaxAge",
												int.class);
								if (artifactMethod != null) {
									artifactMethod.invoke(repository,
											proxyModel.getArtifactMaxAge());
								}

								Method metadataMethod = repository.getClass()
										.getMethod("setMetadataMaxAge",
												int.class);
								if (metadataMethod != null) {
									metadataMethod.invoke(repository,
											proxyModel.getMetadataMaxAge());
								}
							} catch (Exception e) {
								// nothing to do here, doesn't support
								// artifactmax age
							}
						}

						repository.setLocalUrl(model
				...
				ErrorResponse nexusErrorResponse = getNexusErrorResponse("*",
						e.getMessage());
	...
	public void delete(Context context, Request request, Response response)
		...
		try {
			...
			getLogger().warn(
					"Not allowed to delete repository '" + repoId + "'", e);

			throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST,
					"Not allowed to delete repository '" + repoId + "'");
		}
	}
