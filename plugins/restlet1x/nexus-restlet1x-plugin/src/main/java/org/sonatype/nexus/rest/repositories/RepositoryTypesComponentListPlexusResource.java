...
public class RepositoryTypesComponentListPlexusResource extends
		AbstractNexusPlexusResource {
	public static final String RESOURCE_URI = "/components/repo_types";

	private static final Pattern BRACKETS_PATTERN = Pattern
			.compile("(.*)( \\(.*\\))");
	...
	public PathProtectionDescriptor getResourceProtection() {
		return new PathProtectionDescriptor(getResourceUri(),
				"authcBasic,perms[nexus:componentsrepotypes]");
	}
	 ...
	 *
	 * @param repoType
	 *            The type of repository to retrieve providers for. (valid
	 *            values are 'hosted', 'proxy', 'shadow' and 'group').
	 */
	@Override
	@GET
	public Object get(Context context, Request request, Response response,
		...
		// such horrible terminology for this class, its actually repo providers
		...
		for (Template template : templateSet.getTemplatesList()) {
			...
			String providerRole = ((RepositoryTemplate) template)
					.getRepositoryProviderRole();
			String providerHint = ((RepositoryTemplate) template)
					.getRepositoryProviderHint();

			resource.setProvider(providerHint);

			resource.setProviderRole(providerRole);

			resource.setFormat(((AbstractRepositoryTemplate) template)
					.getContentClass().getId());

			// To not disturb the "New repo UI", it's shitty right now: we
			// select templates here that predefines is
