...
public class ArtifactResolvePlexusResource extends
	...
	public PathProtectionDescriptor getResourceProtection() {
		return new PathProtectionDescriptor(getResourceUri(),
				"authcBasic,perms[nexus:artifact]");
	}

	/**
	 * Resolve an artifact identified by GAV coordinates given, and retrieve a
	 * set of details about that artifact.
	 *
	 * @param g
	 ...
	 * @param v
	 *            Version of the artifact (Required) Supports resolving of
	 *            "LATEST", "RELEASE" and snapshot versions ("1.0-SNAPSHOT")
	 *            too.
	 * @param r
	 *            Repository that the artifact is contained in (Required).
	 * @param p
	 *            Packaging type of the artifact (Optional).
	 * @param c
	 *            Classifier of the artifact (Optional).
	 * @param e
	 *            Extension of the artifact (Optional).
	 */
	@Override
	@GET
	public Object get(Context context, Request request, Response response,
		...
		// a flag that will prevent actual download as happens as sideeffect if
		// this is proxy repository
		// and artifact being resolved is not present. This will not affect
		// metadata download, since those are
		...
		try {
			...
			final ArtifactStoreHelper helper = mavenRepository
					.getArtifactStoreHelper();

			// not returning null, will throw INFEx instead
			final Gav resolvedGav = helper.resolveArtifact(gavRequest);

			String repositoryPath = mavenRepository.getGavCalculator()
					.gavToPath(resolvedGav);
			...
			if (resolvedFile != null) {
				resource.setSha1(resolvedFile.getRepositoryItemAttributes()
			...
			if (resource.isSnapshot()) {
				...
				if (resolvedGav.getSnapshotBuildNumber() != null) {
					resource.setSnapshotBuildNumber(resolvedGav
							.getSnapshotBuildNumber());

					resource.setSnapshotTimeStamp(resolvedGav
