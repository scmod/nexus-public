...
/**
 * An {@link PlexusServerServlet} that has an hardcoded name of "nexus" as
 * required by plexus init param lookup.
 *
 ...
 */
@Singleton
class RestletServlet extends PlexusServerServlet {
	private static final long serialVersionUID = -840934203229475592L;

	private static final Logger log = LoggerFactory
			.getLogger(RestletServlet.class);
	...
	public void service(final HttpServletRequest request,
		...
		if (log.isDebugEnabled()) {
			log.debug("Processing: {} {} ({})", request.getMethod(), uri,
		...
		try {
			...
			// special handling for EOF exceptions; tersely log unless debug is
			// enabled
			if (log.isDebugEnabled()) {
	...
	private class DelegatingServletConfig implements ServletConfig {
		...
		public String getInitParameter(String name) {
			return RestletServlet.super.getServletConfig().getInitParameter(
					name);
		}

		@SuppressWarnings("rawtypes")
		public Enumeration getInitParameterNames() {
			return RestletServlet.super.getServletConfig()
	 ...
	 * @param request
	 *            the request to get the server for
	 * @return the {@link Server} from {@link
	 *         super#getServer(HttpServletRequest)} with a custom
